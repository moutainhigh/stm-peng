function Get-NetworkStatistics {
	...
	begin{
        #Define properties
            ...
            $properties = 'Protocol','LocalAddress','RemoteAddress','State'
    	        ...
    	        if($item[1] -notmatch '^\[::'){
                    ...
                    #parse the netstat line for local address and port
    	                if (($la = $item[1] -as [ipaddress]).AddressFamily -eq 'InterNetworkV6'){
    	                    $localAddress = $la.IPAddressToString + ":" + $item[1].split('\]:')[-1]
    	                    #$localPort = $item[1].split('\]:')[-1]
    	                }
    	                else {
    	                    $localAddress = $item[1].split(':')[0] + ":" + $item[1].split(':')[-1]
    	                    #$localPort = $item[1].split(':')[-1]
    	                } 
                
                    #parse the netstat line for remote address and port
    	                if (($ra = $item[2] -as [ipaddress]).AddressFamily -eq 'InterNetworkV6'){
    	                    $remoteAddress = $ra.IPAddressToString + ":" + $item[2].split('\]:')[-1]
    	                    #$remotePort = $item[2].split('\]:')[-1]
    	                }
    	                else {
    	                    $remoteAddress = $item[2].split(':')[0] + ":" + $item[2].split(':')[-1]
    	                    #$remotePort = $item[2].split(':')[-1]
    	                } 
...
#/##
# script output console and FileSystem
#/
function script_printf([string]$contents) {
    #Write-Host $contents
	$contents | out-file -filepath $paramOutFile -append -encoding ASCII
}
