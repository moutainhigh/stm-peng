...
public class HPMSACollector extends SMISCollector {
    Log log = LogFactory.getLog(HPMSACollector.class);

    public HPMSACollector(SMISProvider provider) throws WBEMException {
        super(provider);
        client.initialize(getInitPath(), getSubject(), new Locale[]{Locale.US});
    }

    public HPMSACollector(SMISProvider provider, String name) throws WBEMException, SMISException {
        this(provider);
    ...
    public List<StorageSystem> discoveryStorageSystem() throws WBEMException {
        CIMObjectPath path = new CIMObjectPath(provider.getProtocol(), provider.getIp(), provider.getPort(), provider.getNameSpace(), "HP_TopComputerSystem", null);
    ...
    protected Set<StoragePool> initStoragePool(StorageSystem storageSystem) throws WBEMException {
        ...
        CloseableIterator<CIMInstance> iterator = associatorInstances(storageSystem.getInstance().getObjectPath(), "CIM_HostedStoragePool", "HP_ConcretePool",
    ...
    protected Set<StorageProcessorSystem> initStorageProcessorSystem(StorageSystem storageSystem) throws WBEMException {
        ...
        CloseableIterator<CIMInstance> iterator = associatorInstances(storageSystem.getInstance().getObjectPath(), "CIM_ComponentCS", "HP_Controller",
                "GROUPCOMPONENT", "PARTCOMPONENT", true, null);
        while (iterator.hasNext()) {
        	...
        	storageProcessorSystem.getAdjacentDevices().addAll(initSASPort(storageProcessorSystem));
        	storageProcessorSystem.getAdjacentDevices().addAll(initFCPort(storageProcessorSystem));
        	storageProcessorSystem.getAdjacentDevices().addAll(initDiskExtent(storageProcessorSystem));
    ...
    protected Set<Fan> initFan(StorageSystem storageSystem) throws WBEMException {
        ...
        CloseableIterator<CIMInstance> iterator = associatorInstances(storageSystem.getInstance().getObjectPath(), "CIM_SystemDevice", "HP_PSUFan",
                "GROUPCOMPONENT", "PARTCOMPONENT", true, null);
        while (iterator.hasNext()) {
    ...
    protected Set<Power> initPower(StorageSystem storageSystem) throws WBEMException {
        ...
        CloseableIterator<CIMInstance> iterator = associatorInstances(storageSystem.getInstance().getObjectPath(), "CIM_SystemDevice", "HP_PSU",
                "GROUPCOMPONENT", "PARTCOMPONENT", true, null);
        while (iterator.hasNext()) {
    ...
    protected Set<SASPort> initSASPort(StorageProcessorSystem controller) throws WBEMException {
        HashSet<SASPort> ret = new HashSet<SASPort>();
        CloseableIterator<CIMInstance> iterator = associatorInstances(controller.getInstance().getObjectPath(), "CIM_SystemDevice", "CIM_SASPort",
                "GROUPCOMPONENT", "PARTCOMPONENT", true, null);
        while (iterator.hasNext()) {
            SASPort sasPort = addDevice(SASPort.class, iterator.next());
            sasPort.getAdjacentDevices().add(controller);
            ret.add(sasPort);
        }
        iterator.close();
        return ret;
    ...
    protected Set<FCPort> initFCPort(StorageProcessorSystem controller) throws WBEMException {
        HashSet<FCPort> ret = new HashSet<FCPort>();
        CloseableIterator<CIMInstance> iterator = associatorInstances(controller.getInstance().getObjectPath(), "CIM_SystemDevice", "CIM_FCPort",
                "GROUPCOMPONENT", "PARTCOMPONENT", true, null);
        while (iterator.hasNext()) {
        	FCPort fcPort = addDevice(FCPort.class, iterator.next());
        	fcPort.getAdjacentDevices().add(controller);
            ret.add(fcPort);
        }
        iterator.close();
        return ret;
    ...
    protected Set<DiskExtent> initDiskExtent(StorageProcessorSystem controller) throws WBEMException {
        HashSet<DiskExtent> ret = new HashSet<DiskExtent>();
        CloseableIterator<CIMInstance> iterator = associatorInstances(controller.getInstance().getObjectPath(), "CIM_SystemDevice", "HP_DiskExtent",
                "GROUPCOMPONENT", "PARTCOMPONENT", true, null);
        while (iterator.hasNext()) {
        	DiskExtent diskExtent = addDevice(DiskExtent.class, iterator.next());
        	diskExtent.getAdjacentDevices().add(controller);
            ret.add(diskExtent);
        }
        iterator.close();
        return ret;
