...
import com.mainsteam.stm.plugin.wmi.bean.ResponseCell;
...
/**
 * WMI采集插件，由我们的插件通过socket通道向另一台装有wmi代理的机器发送wql语句，wmi会开启一个监听端口（12345），当它监听到 采集请求时会根据我们的命令去采集，然后再通过socket通道返回回来。
 * 
 * @author xiaopf
 * 
 */
public class WmiSession implements PluginSession {
	...
	private static final String WMIPLUGIN_PASSWORD = "password";
	private static final String WMIPLUGIN_USERNAME = "username";
	private static final String WMIPLUGIN_IP = "IP";
	...
	@Override
	public PluginResultSet execute(PluginExecutorParameter<?> executorParam, PluginSessionContext arg1) throws PluginSessionRunException {
		...
		if (executorParam instanceof PluginArrayExecutorParameter) {
			...
			if (wmiCmd.isEmpty()) {
				throw new PluginSessionRunException(CapcityErrorCodeConstant.ERR_CAPCITY_COLLECT_NULLCOMMAND, "没有wmi采集命令");
			}
			PluginResultSet resultSet = new PluginResultSet();
			List<ArrayList<ArrayList<ResponseCell>>> mutilDatas = new ArrayList<ArrayList<ArrayList<ResponseCell>>>(2);
			for (String cmd : wmiCmd) {
				if (!params.isEmpty()) { // 参数替换
					...
					while (iterator.hasNext()) {
						...
						cmd = StringUtils.replace(cmd, "${" + key + "}", params.get(key));
					}
				}
				try {
			...
			if (logger.isInfoEnabled()) {
				...
				if (null != resultSet) {
					logger.info("return size:" + resultSet.getRowLength() + "," + resultSet.getColumnLength());
				}
			}
			return resultSet;
		} else {
			if (logger.isWarnEnabled()) {
				logger.warn("EMPTY WMI COLLECT PARAMETER" + this.ip + "/" + this.username);
			}
			return null;
		}
	}
	...
	private ArrayList<ArrayList<ResponseCell>> executeCmd(String wmiCmd) {
		...
		request.setWmi_domain_user(this.username);
		request.setWmi_namespace(this.ip);
		request.setWmi_password(this.password);
		request.setWmi_query_cmd(wmiCmd);
		if (!agent.isAvailable())
			agent.start();
		Response response = agent.query(request);
		if (response != null) {
			if (response.getWmi_query_err() != 0) {
				throw new RuntimeException("Query fails!error code:" + response.getWmi_query_err() + "\terror msg:" + response.getWmi_query_msg());
			} else {
				ArrayList<ArrayList<ResponseCell>> wmi_query_data = response.getWmi_query_data();
	...
	public void init(PluginInitParameter init) throws PluginSessionRunException {
		...
		for (int i = 0; i < initParameters.length; i++) {
			...
			default:
				if (logger.isWarnEnabled()) {
					logger.warn("warn:unkown initparameter " + initParameters[i].getKey() + "=" + initParameters[i].getValue());
				}
				break;
			}
		}
		try {
			agent = WMIAgentFactory.getDefaultAgent();
			...
			if (StringUtils.equals("Unable to connect Wmi Agent", e.getMessage())) {
	 ...
	 *            返回ResultSet
	 */
	private PluginResultSet putValues(ArrayList<ArrayList<ResponseCell>> data, PluginResultSet resultSet) {
		// 查询出最长的一行
		int maxlength = 0;
				...
				try {
					...
					for (int j = 0; j < maxlength; j++) {
						if (j > childs.size())
							...
							if (cell != null) {
								List<String> finalValue = cell.getWmi_res_value();
								try {
	 ...
	 * 根据两个结果集中两个相同的列进行合并
	 */
	private void mergeResultSet(List<ArrayList<ArrayList<ResponseCell>>> result, PluginResultSet resultSet, List<String> columns) {
		...
		for (ArrayList<ResponseCell> firstRows : firstResult) {
				...
				if (StringUtils.equalsIgnoreCase(firstCol, column)) { // 在第一个结果集中遍历到相同列名
					...
					for (ArrayList<ResponseCell> secondRows : secondResult) {
						for (ResponseCell cell2 : secondRows) {
							...
							if (StringUtils.equalsIgnoreCase(otherColumn, secondCol)) { // 在第二个结果集中找到相同的列
								List<String> otherStrs = cell2.getWmi_res_value();
								if (otherStrs == null)
									break;
								if (StringUtils.equals(strs.toString(), otherStrs.toString())) { // 两列值相同
									flag = true;
									break;
								}
							}
			...
			if (flag) { // 先添加第一个结果集，然后添加另外一个数据集匹配的行数
				try {
					...
					ArrayList<ResponseCell> otherCells = secondResult.get(commonRow);
