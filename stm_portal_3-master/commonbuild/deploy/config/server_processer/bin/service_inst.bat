...
set "SERVER_HOME=%cd%"
if exist "%SERVER_HOME%\bin\server_dhs.exe" goto okHome
rem CD to the upper dir
cd ..
set "SERVER_HOME=%cd%"
:gotHome
if exist "%SERVER_HOME%\bin\server_dhs.exe" goto okHome
echo The server_dhs.exe was not found...
...
set "EXECUTABLE=%SERVER_HOME%\bin\server_dhs.exe"

rem Set default Service name
set SERVICE_NAME=<OC4_DHS_SERVICE_NAME>
set DISPLAYNAME=<OC4_DHS_SERVICE_DISPLAY>
...
"%EXECUTABLE%" //IS//%SERVICE_NAME% ^
    ...
    --StartParams "" ^
    --StopParams stop ^
    --JvmOptions "-XX:+HeapDumpOnOutOfMemoryError;-DCLASS_PATH=%CLASS_PATH%;-DSTARTUP_CLASS=%STARTUP_CLASS%;-DJAR_FILE_PATH=%JAR_FILE_PATH%;-DserverType=%serverType%;-Dcaplibs.path=%caplibs.path%" ^
    --JvmMs 768 ^
    --JvmMx 1024
