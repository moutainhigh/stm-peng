 ...
 * 信息指标告警
 */
@Component("infoMetricProcessor")
public class InfoMetricProcessor extends AbstractMetricCompute implements StateProcessor {

    private static final Log logger = LogFactory.getLog(InfoMetricProcessor.class);

    @Autowired
    @Qualifier("infoMetricComputeAlgorithm")
    ...
    public Object process(StateComputeContext context) {
        ...
        if(additions.containsKey("skipMetricFlapping")){ //跳过Flapping计算
    ...
    public Object compute(ProfileThreshold profileThreshold, StateComputeContext context) throws Exception {
        MetricCalculateData metricData = context.getMetricData();
        return metricCompute.compute(metricData.getMetricData()[0], profileThreshold.getMetricId(),
                profileThreshold.getThresholdExpression(), metricData.getResourceInstanceId(), metricData.isCustomMetric());
    }
    ...
    public String toString() {
        return "InfoMetricProcessor{"+processOrder()+"}";
    }
}
