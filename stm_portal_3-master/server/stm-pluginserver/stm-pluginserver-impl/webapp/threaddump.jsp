...
<%!public String dump() {
	...
	long tlCount = threadMBean.getThreadCount();
	long tpCount = threadMBean.getPeakThreadCount();
	long ttCount = threadMBean.getTotalStartedThreadCount();
	StringBuilder sb = new StringBuilder();
	sb.append(" ThreadCount=").append(tlCount).append(" PeakThreadCount=")
			.append(tpCount).append(" TotalStartedThreadCount=")
			.append(ttCount).append('\n').append('\n').append('\n');
	if (threadMBean.isObjectMonitorUsageSupported()) {
		ThreadInfo[] infos = threadMBean.dumpAllThreads(true, false);
		for (ThreadInfo threadInfo : infos) {
			appendThreadInfo(sb, threadInfo, threadInfo.getLockedMonitors());
		}
	} else {
		long[] threads = threadMBean.getAllThreadIds();
		long currentThId = Thread.currentThread().getId();
		MonitorInfo[] monitors = null;
		for (long newThread : threads) {
			if (newThread == currentThId) {
				continue;
			}
			ThreadInfo ti = threadMBean.getThreadInfo(newThread,
					Integer.MAX_VALUE);
			monitors = ti.getLockedMonitors();
			appendThreadInfo(sb, ti, monitors);
		}
	}
	return sb.toString();
}

	private void appendThreadInfo(StringBuilder sb, ThreadInfo ti,
			MonitorInfo[] monitors) {
		if (ti != null) {
			sb.append("\n\n-----------------------------------------\n");
			sb.append("ThreadId:").append(ti.getThreadId()).append("\n");
			if (ti.getLockName() == null) {
				sb.append(Resources.getText("Name State", ti.getThreadName(),
						ti.getThreadState().toString()));
			} else if (ti.getLockOwnerName() == null) {
				sb.append(Resources.getText("Name State LockName",
						ti.getThreadName(), ti.getThreadState().toString(),
						ti.getLockName()));
			} else {
			...
			sb.append(Resources.getText("BlockedCount WaitedCount",
					ti.getBlockedCount(), ti.getWaitedCount()));
				...
				if (monitors != null) {
					for (MonitorInfo mi : monitors) {
						if (mi.getLockedStackDepth() == index) {
							sb.append(Resources.getText("Monitor locked",
									mi.toString()));
						}
					}
				}
...
<%!public static class JConsoleResources extends ListResourceBundle {
		...
		protected Object[][] getContents0() {
			return new Object[][] {
					...
					{ "Array, OpenType, Numeric value viewer",
							"Array, OpenType, Numeric value viewer" },
					{ "Attribute", "Attribute" },
					...
					{ "Class", "Class" },
					{ "ClassName", "ClassName" },
					{ "ClassTab.infoLabelFormat",
							"<html>Loaded: {0}    Unloaded: {1}    Total: {2}</html>" },
					{ "ClassTab.loadedClassesPlotter.accessibleName",
							"Chart for Loaded Classes." },
					{ "Classes", "Classes" },
					{ "Close", "Close" },
					{ "Column.Name", "Name" },
					{ "Column.PID", "PID" },
...

					{
							...
							"{0,choice,1#{0,number,integer} hour |1<{0,number,integer} hours }"
					...
					{ "Exit.mnemonic", 'x' },
					{ "Fail to load plugin",
							"Warning: Fail to load plugin: {0}" },
					{ "FileChooser.fileExists.cancelOption", "Cancel" },
					{ "FileChooser.fileExists.message",
					...
					{ "FileChooser.savedFile",
							"<html>Saved to file:<br>{0}<br>({1} bytes)" },
					{ "FileChooser.saveFailed.message",
							"<html><center>Save to file failed:<br>{0}<br>{1}" },
					...
					{ "Help.AboutDialog.userGuideLink",
							"JConsole User Guide:<br>{0}" },
					{ "Help.AboutDialog.userGuideLink.mnemonic", 'U' },
					{ "Help.AboutDialog.userGuideLink.url",
							"http://java.sun.com/javase/6/docs/technotes/guides/management/jconsole.html" },
					{ "Help.AboutDialog.userGuideLink.linkText",
							"Link to the JConsole User's Guide" },
					...
					{ "Hotspot MBeans.dialog.accessibleDescription",
							"Dialog for managing Hotspot MBeans" },
					{ "Impact", "Impact" },
					{ "Info", "Info" },
					{ "INFO", "INFO" },
					{ "Invalid plugin path",
							"Warning: Invalid plugin path: {0}" },
					...
					{ "Monitor locked", "   - locked {0}" + cr },
					{ "Motif", "Motif" },
					{ "Name Build and Mode", "{0} (build {1}, {2})" },
					{ "Name and Build", "{0} (build {1})" },
					{ "Name", "Name" },
					{ "Name: ", "Name: " },
					{ "Name State", "Name: {0}" + cr + "State: {1}" + cr },
					{ "Name State LockName",
							"Name: {0}" + cr + "State: {1} on {2}" + cr },
					{
					...
					{ "TabularData are not supported",
							"TabularData are not supported" },
					{ "Threads", "Threads" },
					{ "ThreadTab.infoLabelFormat",
							"<html>Live: {0}    Peak: {1}    Total: {2}</html>" },
					...
					{ "Verbose Output.toolTip",
							"Enable verbose output for class loading system" },
					...
					{
							"connectionFailed2",
							"The connection to {0} did not succeed.<br>"
					...
					{
							"connectingTo2",
							"You are currently being connected to {0}.<br>"
	...
	public static final class Resources {
		...
		private static JConsoleResources resources = new JConsoleResources();
