...
public class ProfileDispatchTimer {

	private static final Log logger = LogFactory
			.getLog(ProfileDispatchTimer.class);
	...
	public void start() {
		...
		Timer instanceTimer = new Timer("ProfileDispatchTimer");
	...
	private void deleteTimelines(
		...
		// key: groupNodeId
		Set<String> groupNodeIds = getNodeGroupsByDHS();
		if (groupNodeIds == null || groupNodeIds.isEmpty()) {
			return;
		}
		ProfileDeployInfo info = new ProfileDeployInfo();
		info.setAction(profileChange.getProfileChangeEnum().name());
	...
	private Set<String> getNodeGroupsByDHS(){
		return allNodeGroupIds.keySet();
	}
...
//	}
	...
	private void setService() {
		...
		if(customMetricChangeService == null){
	...
	private void cancelInstance(Map<String, ProfileDeployInfoWrapper> profileDeployMap,
			...
			Map<Long, Map<String, Boolean>> changeResultsMap,
			List<Long> invalidChangeSateIds){
		// key：nodegroupId value: 删除监控同步到采集器 malachi in
				long instanceId = Long.parseLong(profileChange.getSource());
				ResourceInstance resourceInstance = getResourceInstance(instanceId);
				String nodeGroupId = null;
				if (resourceInstance == null) {
	...
	public void execute() throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("Profile Deploy Timer start.");
		}
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Profile Deploy Timer:" + new Date());
		}
	}
