...
/**
 * 删除资源实例拦截类（同步处理器资源到采集器）
 * @author xiaoruqiang
 */
public class InstancelibInterceptorRefreshImpl implements InstancelibInterceptor {

	private static final Log logger = LogFactory
			.getLog(InstancelibInterceptorRefreshImpl.class);
	...
	public void interceptor(InstancelibEvent instancelibEvent) {
		if(instancelibEvent.getEventType() == EventEnum.INSTANCE_REFRESH_EVENT){
			List<ResourceInstance> resourceInstances = (List<ResourceInstance>)instancelibEvent.getCurrent();
			if(logger.isTraceEnabled()){
				logger.trace("refresh resource instance interceptor start =" + resourceInstances);
			}
				...
				if(node == null){
					...
					try {
						...
						if(collectorResourceInstanceSyncMBean == null){
							if(logger.isWarnEnabled()){
								logger.warn("refresh resource remote collector MBen not found!");
							}
						}else{
							collectorResourceInstanceSyncMBean.refreshResourceInstancesSyncToCollector(temp.getValue());
						}
					} catch (Exception e) {
						if(logger.isErrorEnabled()){
							logger.error("refresh resourceinstance sync to collector error!", e);
						}
					}
			    }
			}
			if(logger.isTraceEnabled()){
				logger.trace("batch refresh resource instance interceptor end  =" + resourceInstances );
			}
		}
	}
