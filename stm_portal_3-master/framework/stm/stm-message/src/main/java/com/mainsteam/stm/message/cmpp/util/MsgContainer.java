...
import com.mainsteam.stm.message.cmpp.domain.MsgConnect;
import com.mainsteam.stm.message.cmpp.domain.MsgHead;
import com.mainsteam.stm.message.cmpp.domain.MsgSubmit;

/**
 * <li>文件名称: AlarmInfo.java</li> <li>公　　司: 武汉美新翔盛科技有限公司</li> <li>版权所有:
 * 版权所有(C)2019-2020</li> <li>修改记录: ...</li> <li>内容摘要: 短信接口容器，单例获得链接对象</li> <li>
 * 其他说明: ...</li>
 * 
 * @version ms.stm
 * @since 2019年8月18日
...
public class MsgContainer {
	...
	public static Socket getSocketInstance() {
		if (null == msgSocket || msgSocket.isClosed()
			...
			try {
				...
				throw new RuntimeException("Socket链接短信网关端口号不正确：" + e.getMessage());
				// 链接短信网关
			} catch (IOException e) {
				e.printStackTrace();
				logger.error("Socket链接短信网关失败：" + e.getMessage());
				throw new RuntimeException("Socket链接短信网关失败：" + e.getMessage());
			}
		}
		return msgSocket;
	}
	...
	public static boolean sendMsg(String msg, List<String> cusMsisdn) {
		logger.info("CMPP准备发送短信！");
		try {
			if (msg.getBytes("utf-8").length < 140) {// 短短信
				boolean result=false;
				for (String mobile : cusMsisdn) {
					result = sendShortMsg(msg, mobile);
					int count = 0;
					while (!result) {
						count++;
						result = sendShortMsg(msg, mobile);
	...
	public static boolean sendWapPushMsg(String url, String desc,
		...
		try {
			...
			throw new RuntimeException("发送web push短信:" + e.getMessage());
		}
	}
	...
	private static boolean sendShortMsg(String msg, String cusMsisdn) {
		try {
			...
			throw new RuntimeException("发送短短信" + e.getMessage());
		}
	}
	...
	private static boolean sendLongMsg(String msg, String cusMsisdn) {
		try {
			...
			throw new RuntimeException("发送长短信" + e.getMessage());
		}
	}
	...
	public static boolean cancelISMG() {
		try {
			...
			throw new RuntimeException("拆除与ISMG的链接" + e.getMessage());
		}
	}
	...
	public static boolean activityTestISMG() {
		try {
			...
			throw new RuntimeException("链路检查" + e.getMessage());
		}
	}
	...
	private static boolean sendShortWapPushMsg(String url, String desc,
		...
		try {
			...
			throw new RuntimeException("发送web push短短信" + e.getMessage());
		}
	}
	...
	private static boolean sendLongWapPushMsg(String url, String desc,
		...
		try {
			...
			throw new RuntimeException("发送web push长短信" + e.getMessage());
		}
	}
}
