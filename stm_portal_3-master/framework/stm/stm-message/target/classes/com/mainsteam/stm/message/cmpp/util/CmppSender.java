...
import com.mainsteam.stm.message.cmpp.domain.MsgActiveTestResp;
import com.mainsteam.stm.message.cmpp.domain.MsgCommand;
import com.mainsteam.stm.message.cmpp.domain.MsgConnectResp;
import com.mainsteam.stm.message.cmpp.domain.MsgDeliver;
import com.mainsteam.stm.message.cmpp.domain.MsgDeliverResp;
import com.mainsteam.stm.message.cmpp.domain.MsgHead;
import com.mainsteam.stm.message.cmpp.domain.MsgSubmitResp;


/**
 * <li>文件名称: CmppSender.java</li>
 * <li>公　　司: 武汉美新翔盛科技有限公司</li>
 * <li>版权所有: 版权所有(C)2019-2020</li>
 * <li>修改记录: ...</li>
 * <li>内容摘要: 启动一个线程去接收和发送数据，如果队列处理完毕就关闭线程</li>
 * <li>其他说明: ...</li>
 * @version  ms.stm
 * @since   2019年8月18日
 * @author   zhangjunfeng
				...
				if(data.length>=8){
					...
					switch(head.getCommandId()){
						case MsgCommand.CMPP_CONNECT_RESP:
							...
							logger.info("ms.stm于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"链接短信网关,状态:"+connectResp.getStatusStr()+" 序列号："+connectResp.getSequenceId());
							break;
						case MsgCommand.CMPP_ACTIVE_TEST_RESP:
							MsgActiveTestResp activeResp=new MsgActiveTestResp(data);
							logger.info("ms.stm于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"短信网关与短信网关进行连接检查"+" 序列号："+activeResp.getSequenceId());
							break;
						case MsgCommand.CMPP_SUBMIT_RESP:
							MsgSubmitResp submitResp=new MsgSubmitResp(data);
							logger.info("ms.stm于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向用户下发短信，状态码:"+submitResp.getResult()+" 序列号："+submitResp.getSequenceId());
							break;
						case MsgCommand.CMPP_TERMINATE_RESP:
							logger.info("ms.stm于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"拆除与ISMG的链接"+" 序列号："+head.getSequenceId());
							break;
