...
import com.mainsteam.stm.portal.netflow.api.ipgroup.netflow.IIpGroupTosApi;
...
/**
 * <li>文件名称: IpGroupTosAction.java</li> <li>公　　司: 武汉美新翔盛科技有限公司</li> <li>版权所有:
 ...
 */
@Controller
@RequestMapping("/netflow/ipgroup/tos")
public class IpGroupTosAction extends BaseAction {

	@Autowired
	private IIpGroupTosApi ipGroupTosApi;

	@RequestMapping("/ipGroupTosPageSelect")
	public JSONObject ipGroupTosPageSelect(GenericNetflowVo vo) {
		NetflowParamBo bo = NetflowActionUtil.toBo(vo);
		BaseAction.getHttpServletRequest().getSession()
				.setAttribute(IP_GROUP_TOS_NETFLOW_TABLE_THREE, bo);
		NetflowPageBo ret = ipGroupTosApi.ipGroupTosPageSelect(bo);
		ret.setParamBo(bo);
		BaseAction
				.getHttpServletRequest()
				.getSession()
				.setAttribute(IPGroupExportAction.IP_GROUP_TOS_TABLE_THREE, ret);
	...
	public JSONObject getIpGroupAppChartData(GenericNetflowVo vo) {
		...
		NetflowCharWrapper ret = ipGroupTosApi.getIpGroupAppChartData(bo);
		ret.setSortColumn(bo.getSort());
		BaseAction
				.getHttpServletRequest()
				.getSession()
				.setAttribute(IPGroupExportAction.IP_GROUP_TOS_CHART_THREE, ret);
		return toSuccess(ret);
	}

	private static final String IP_GROUP_TOS_NETFLOW_TABLE_THREE = "IP_GROUP_TOS_NETFLOW_TABLE_THREE";

	@RequestMapping("exportTos")
	public void exportTos(HttpServletResponse response, String type)
			throws IOException, NoSuchMethodException, SecurityException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException {
		NetflowParamBo bo = (NetflowParamBo) BaseAction.getHttpServletRequest()
				.getSession().getAttribute(IP_GROUP_TOS_NETFLOW_TABLE_THREE);
		NetflowPageBo ret = ipGroupTosApi.ipGroupTosPageSelect(bo);
		bo.setStartRow(0);
		...
		pdf.addTable(new TableAdapter().adapter("TOS", getColumnTos(type),
				ret.getRows()));
		DownUtil.down(response, "ip_group_tos.pdf", pdf.generate());
	}
