...
import com.mainsteam.stm.portal.netflow.api.ipgroup.netflow.IIpGroupProtoApi;
...
/**
 * <li>文件名称: IpGroupProtoAction.java</li> <li>公　　司: 武汉美新翔盛科技有限公司</li> <li>版权所有:
 ...
 */
@Controller
@RequestMapping("/netflow/ipgroup/proto")
public class IpGroupProtoAction extends BaseAction {

	@Autowired
	private IIpGroupProtoApi ipGroupProtoApi;

	@RequestMapping("/ipGroupProtoPageSelect")
	public JSONObject ipGroupProtoPageSelect(GenericNetflowVo vo) {
		NetflowParamBo bo = NetflowActionUtil.toBo(vo);
		BaseAction.getHttpServletRequest().getSession()
				.setAttribute(IP_GROUP_PROTOCOL_NETFLOW_TABLE_THREE, bo);
		NetflowPageBo ret = ipGroupProtoApi.ipGroupProtoPageSelect(bo);
		ret.setParamBo(bo);
		BaseAction
				.getHttpServletRequest()
				.getSession()
				.setAttribute(
						IPGroupExportAction.IP_GROUP_PROTOCOL_TABLE_THREE, ret);
		return toSuccess(ret);
	}

	@RequestMapping("/getIpGroupProtoChartData")
	public JSONObject getIpGroupProtoChartData(GenericNetflowVo vo) {
		NetflowParamBo bo = NetflowActionUtil.getBo(vo);
		NetflowCharWrapper ret = ipGroupProtoApi.getIpGroupProtoChartData(bo);
		return toSuccess(ret);
	}

	private static final String IP_GROUP_PROTOCOL_NETFLOW_TABLE_THREE = "IP_GROUP_PROTOCOL_NETFLOW_TABLE_THREE";

	@RequestMapping("exportProto")
	public void exportProto(HttpServletResponse response, String type)
			throws IOException, NoSuchMethodException, SecurityException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException {
		NetflowParamBo bo = (NetflowParamBo) BaseAction.getHttpServletRequest()
				.getSession()
				.getAttribute(IP_GROUP_PROTOCOL_NETFLOW_TABLE_THREE);
		bo.setStartRow(0);
		...
		pdf.addTable(new TableAdapter().adapter("协议", getColumnProto(type),
				ret.getRows()));
		DownUtil.down(response, "ip_group_proto.pdf", pdf.generate());
	}
