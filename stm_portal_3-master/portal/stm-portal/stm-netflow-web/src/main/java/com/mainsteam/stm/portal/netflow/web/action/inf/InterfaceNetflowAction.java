...
/**
 * <li>文件名称: InterfaceNetflowAction.java</li> <li>公　　司: 武汉美新翔盛科技有限公司</li> <li>
 ...
 */
@Controller
@RequestMapping("/netflow/if")
public class InterfaceNetflowAction extends BaseAction {

	@Autowired
	private IIfNetflowApi ifNetflowApi;

	@RequestMapping("/ifListPageSelect")
	public JSONObject ifListPageSelect(GenericNetflowVo vo) {
		NetflowParamBo bo = NetflowActionUtil.toBo(vo);
		BaseAction.getHttpServletRequest().getSession()
				.setAttribute(EXPORT_INTERFACE_NETFLOW_TABLE_QUERY, bo);
		NetflowPageBo ret = ifNetflowApi.ifListPageSelect(bo);
		ret.setParamBo(bo);
		BaseAction.getHttpServletRequest().getSession()
				.setAttribute(EXPORT_INTERFACE_NETFLOW_TABLE, ret);
		return toSuccess(ret);
	}

	@RequestMapping("/getIfNetflowChartData")
	public JSONObject getIfNetflowChartData(GenericNetflowVo vo) {
		NetflowParamBo bo = NetflowActionUtil.getBo(vo);
		NetflowCharWrapper ret = ifNetflowApi.getIfNetflowChartData(bo);
		BaseAction.getHttpServletRequest().getSession()
				.setAttribute(EXPORT_INTERFACE_NETFLOW_CHART, ret);
		return toSuccess(ret);
	}

	private static final String EXPORT_INTERFACE_NETFLOW_CHART = "EXPORT_INTERFACE_NETFLOW_CHART";
	private static final String EXPORT_INTERFACE_NETFLOW_TABLE = "EXPORT_INTERFACE_NETFLOW_TABLE";
	private static final String EXPORT_INTERFACE_NETFLOW_TABLE_QUERY = "EXPORT_INTERFACE_NETFLOW_TABLE_QUERY";
	...
	public void exportAll(HttpServletResponse response, String type)
		...
		NetflowParamBo bo = (NetflowParamBo) BaseAction.getHttpServletRequest()
				.getSession()
				.getAttribute(EXPORT_INTERFACE_NETFLOW_TABLE_QUERY);
		bo.setStartRow(0);
		bo.setRowCount(Long.MAX_VALUE);
		NetflowPageBo pageBo = ifNetflowApi.ifListPageSelect(bo);
		pageBo.setParamBo(bo);
		PDFHelper pdf = new PDFHelper("接口流量");
		pdf.addTable(new TableAdapter().adapter("接口流量数据", getColumn(type),
				pageBo.getRows()));
		DownUtil.down(response, "infaces.pdf", pdf.generate());
	}
	...
	public void export(HttpServletResponse response, String type)
		...
		NetflowCharWrapper chart = (NetflowCharWrapper) BaseAction
				...
				.getAttribute(EXPORT_INTERFACE_NETFLOW_CHART);
		PDFHelper pdf = new PDFHelper("接口流量");
		pdf.addLine("接口流量折线图", "时间", "流量(MB)", new LineAdapter().adapter(chart));
		NetflowPageBo pageBo = (NetflowPageBo) BaseAction
				.getHttpServletRequest().getSession()
				.getAttribute(EXPORT_INTERFACE_NETFLOW_TABLE);
		pdf.addCircular("接口流量环形图", new CircularAdapter().adapter("ifName",
	...
	private ShowColumn[] getColumn(String type) {
		...
		case "1":
			ShowColumn[] data1 = {
					new ShowColumn("ifName", "接口名称", ShowColumnType.OTHER),
