...
import com.mainsteam.stm.portal.netflow.api.inf.IIfTerminalApi;
 ...
 */
@Controller
@RequestMapping("/netflow/if/terminal")
public class InterfaceTerminalAction extends BaseAction {

	@Autowired
	private IIfTerminalApi ifTerminalApi;

	/**
	 * 处理查询接口终端流量的请求-datagrid
	 */
	@RequestMapping("ifTerminalPageSelect")
	public JSONObject ifTerminalPageSelect(GenericNetflowVo vo) {
		NetflowParamBo bo = NetflowActionUtil.toBo(vo);
		BaseAction.getHttpServletRequest().getSession()
				.setAttribute(INTERFACE_TERMINAL_TABLE_THREE, bo);
		NetflowPageBo ret = ifTerminalApi.ifTerminalPageSelect(bo);
		ret.setParamBo(bo);
		BaseAction
				.getHttpServletRequest()
				.getSession()
				.setAttribute(TwoExportAction.INTERFACE_TERMINAL_TABLE_TWO, ret);
		return toSuccess(ret);
	}

	/**
	 * 处理查询接口终端流量的请求-highcharts
	 */
	@RequestMapping("/getIfTerminalChartData")
	public JSONObject getIfTerminalChartData(GenericNetflowVo vo) {
		NetflowParamBo bo = NetflowActionUtil.getBo(vo);
		NetflowCharWrapper ret = ifTerminalApi.getIfTerminalChartData(bo);
		return toSuccess(ret);
	}

	private static final String INTERFACE_TERMINAL_TABLE_THREE = "INTERFACE_TERMINAL_TABLE_THREE";

	@RequestMapping("interfaceTerminal")
	public void interfaceTerminal(HttpServletResponse resp, String type)
			throws IOException, NoSuchMethodException, SecurityException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException {
		NetflowParamBo bo = (NetflowParamBo) BaseAction.getHttpServletRequest()
				.getSession().getAttribute(INTERFACE_TERMINAL_TABLE_THREE);
		bo.setStartRow(0);
		...
		pdf.addTable(new TableAdapter().adapter("终端", getColumnTerminal(type),
				ret.getRows()));
		DownUtil.down(resp, "interface_terminal.pdf", pdf.generate());
	}
