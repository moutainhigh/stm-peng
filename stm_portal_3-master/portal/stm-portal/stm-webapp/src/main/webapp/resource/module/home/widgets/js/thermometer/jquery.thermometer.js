	...
	var Thermometer = {
		...
		*/
		setTitle : function(newText) {
			...
			//this.title.attr('dx',(420-255)-w/2);
		},
		/**
		* set title
		*/
		setSubTitle : function(newText) {
			this.subTitle.text(newText);
			/*var _this = this;
			setTimeout(function(){
				var w = _this.subTitle.width();
				_this.subTitle.attr('dx',(420-255)-w/2);
				console.log(w);
			},100);//*/
		...
		_updateLiquidColour: function() {
			...
			this._formatDataAttribute( this.liquidLableLine, "style", variables );
		},
		...
		_setupSVGLinks: function(svg) {
			...
			this.liquidLable =  $('#'+id+'_liquidLable');
			this.liquidLableLine =  $('#'+id+'_liquidLable polyline');
			this.liquidLableText =  $('#'+id+'_liquidLable text');
			
			this.title =  $('#'+id+'_title');
			this.subTitle =  $('#'+id+'_subTitle');
		...
		_create: function() {
			...
			div.load( this.options.pathToSVG, null, function() {
				...
				svg[0].setAttribute( "viewBox", self.leftOffset+" "+self.topOffset+" 700 800" );
				...
				self.setSubTitle(self.options.subTitle);

				var mg = (div.parent().height() - svg.height())/2;
				mg = (mg>0)?mg:0;
				svg.css({'margin-left': 'auto', 'margin-right': 'auto', 'margin-top':mg+'px', display: 'block'});
