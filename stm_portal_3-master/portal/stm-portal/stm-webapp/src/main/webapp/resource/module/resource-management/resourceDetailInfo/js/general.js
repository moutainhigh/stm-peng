...
//					json = {"data":[[[{"id":"statusLight","unit":"","colspan":"1","name":"","width":"4%","value":"UNKNOWN_NOTHING","rowspan":"2","type":"text"},{"id":null,"colspan":"1","width":"10.5%","value":"资源名称","rowspan":"1","type":"text"},{"id":"name","colspan":"1","width":"12.5%","value":"52A18956","rowspan":"1","type":"text"},{"id":null,"colspan":"1","width":"12.5%","value":"显示名称","rowspan":"1","type":"text"},{"id":"showname","colspan":"1","width":"12.5%","value":"52A18956","rowspan":"1","type":"text"},{"id":null,"colspan":"1","width":"12.5%","value":"操作系统","rowspan":"1","type":"text"},{"id":"osVersion","unit":"","colspan":"1","name":"","width":"12.5%","value":"","rowspan":"1","type":"text"},{"id":null,"colspan":"1","width":"12.5%","value":"IP地址","rowspan":"1","type":"text"},{"id":"ip","colspan":"1","width":"12.5%","value":[{"id":"10.154.8.20","name":"10.154.8.20"}],"rowspan":"1","type":"text"}],[{"id":null,"colspan":"1","width":"12.5%","value":"当前可用性","rowspan":"1","type":"text"},{"id":"availability","unit":"","colspan":"1","name":"","width":"12.5%","value":"UNKNOWN_NOTHING","rowspan":"1","type":"text"},{"id":null,"colspan":"1","width":"12.5%","value":"运行时长","rowspan":"1","type":"text"},{"id":"sysUpTime","unit":"","colspan":"1","name":"","width":"12.5%","value":"","rowspan":"1","type":"text"},{"id":null,"colspan":"1","width":"12.5%","value":"所属域","rowspan":"1","type":"text"},{"id":"domain","colspan":"1","width":"12.5%","value":"默认域","rowspan":"1","type":"text"},{"id":null,"colspan":"1","width":"12.5%","value":"当前策略","rowspan":"1","type":"text"},{"id":"profile","colspan":"1","width":"12.5%","value":"默认策略VCS Control","rowspan":"1","type":"text"}]],[[{"id":null,"unit":"","title":"Traversal","colspan":"1","name":"","width":"33%","value":[{"metricStatus":"NORMAL","value":"0","metricStr":"TraversalCurrent","metricName":"当前值"},{"metricStatus":"NORMAL","value":"0","metricStr":"TraversalMax","metricName":"峰值"},{"metricStatus":"NORMAL","value":"0","metricStr":"TraversalTotal","metricName":"重启后累计值"}],"rowspan":"1","type":"metricTabs"},{"id":null,"unit":"","title":"Non-Traversal","colspan":"1","name":"","width":"33%","value":[{"metricStatus":"NORMAL","value":"0","metricStr":"NonTraversalCurrent","metricName":"当前值"},{"metricStatus":"NORMAL","value":"3","metricStr":"NonTraversalMax","metricName":"峰值"},{"metricStatus":"NORMAL","value":"12","metricStr":"NonTraversalTotal","metricName":"重启后累计值"}],"rowspan":"1","type":"metricTabs"},{"id":null,"unit":"","title":"Registrations","colspan":"1","name":"","width":"33%","value":[{"metricStatus":"NORMAL","value":"11","metricStr":"RegistrationsCurrent","metricName":"当前值"},{"metricStatus":"NORMAL","value":"14","metricStr":"RegistrationsMax","metricName":"峰值"},{"metricStatus":"NORMAL","value":"78","metricStr":"RegistrationsTotal","metricName":"重启后累计值"}],"rowspan":"1","type":"metricTabs"}],[{"id":null,"unit":"","title":"指标一览","colspan":"3","name":"","width":"100%","childtype":["prefmetric","infometric"],"rowspan":"1","type":"tabs"}]]],"code":200};
		...
		createTable : function(tabData, tabNo){
			...
			if(tabNo == 0){
				tabOutDom.find("td:odd").each(function(){
		...
		createTd : function(trDom, tdData, tdSize){
			var that = this;
			// 布局
			var tdDom = $("<td/>").width(tdData.width).attr('colspan', tdData.colspan);
			...
			case "progress":
				...
				break;
			case "multiMeter":case "fan":case "cpu":case "node":case "battery":case "camera":
		...
		createText : function(tdDom, tdData){
			...
			if(tdData.id == 'statusLight'){
				tdDom.append(this.returnStatusLight(tdData.value, 'mainInstance'));
			}
			else if(tdData.id == 'liablePerson')
			{
				oc.util.ajax({
					...
					success:function(d){
						...
						if(liableUser.name.length > 4){
							tdHtml = liableUser.name.substring(0, 5)+ "...";
							var content = $("<span/>").css({
								"white-space":"nowrap",
								"width":"40"
							}).html(tdHtml).attr("title",liableUser.name);
						...
						//	tdDom.html(content);		
							var liablePersonBtn = $("<span/>").addClass('ico ico-see').css('float','right');						
							tdDom.append(liablePersonBtn);	
						       var tooltipHtml = '<div id="tooltip" class="tooltip_selfdefine">'
						    		+'<form>'
				...
				var editBtn = $("<span/>").addClass('uodatethetext').attr('title', '编辑名称');//.css('float','right')
				var editNode = $("<input name='editName' type='text'>").css('float','left').width('100px');
				editNode.validatebox({
					validType : 'maxLength[30]'
				});
				var showNode = $("<div name='showName' />").css('float','left').addClass('generalShowName')
								.attr('title', that.showName).html(that.showName.htmlspecialchars()).width('100px');
				...
				editBtn.on('click', function() {
					if (that.flag) {
						...
						showNode.html('').hide();
						editBtn.attr('title', '确认名称修改').removeClass().addClass('uodatethetextok');
						that.flag = false;
					} else {
									...
									success : function(json){
										if(json.code == 200){
											if(json.data == 0){
												...
												editNode.val('').hide();
												editBtn.attr('title', '编辑名称').removeClass().addClass('uodatethetext');
												that.flag = true;
...
//													tdDom.find('#tvImgP').hide();
				...
				var content = $("<div/>").css({
					"white-space":"nowrap"
				}).html(tdData.value == undefined || tdData.value == null ? '' : tdHtml)
...
//			}
			var subTitle = '';
			if(tdData.totalMemSize != undefined && tdData.memPoolUsed != undefined){
				//subTitle = "<span class='NHmemSize'>内存总容量：" + tdData.totalMemSize + "</span><span class='NHmemPooluesd'>内存已使用容量：" + tdData.memPoolUsed + "</span>";
			...
			progress.highcharts({
				...
				chart.renderer.rect(24, 51, width, height, 5)
	            .attr({
	                'stroke-width': 2,
		...
		createGraph : function(tdDom, tdData, tdSize){
			...
			var step = tdData.historyDataColTim.length > 4 ? Math.floor(tdData.historyDataColTim.length / 4) : null;
			graph.highcharts({
	            chart: {
	                type: 'area',
	                margin : [10, 20, 25, 57],
...
//	                	step : step,
		...
		createChildrenRes : function(tdDom, tdData, tdSize){
			...
			d_page.find("a").on('click', function(){
				...
				that.changePageContent(currentA, 4, tdData.type, tdData.value, higContainer,tdData.childtype)
			});
			// 如果只有一页则隐藏翻页按钮
			if(pageNum == 1){
				d_page.hide();
			...
			// 初始化第一页的显示
			this.createPageChildRes(1, 4, tdData.type, tdData.value, higContainer,tdData.childtype);
			
		},
		changePageContent : function(currentA, pageSize, type, value, container,childtype){
			var that = this;
			currentA.removeClass("d-page-dot").addClass("d-page-greendot");
			container.fadeOut('fast', function(){
				that.createPageChildRes(parseInt(currentA.attr('pageNo')), pageSize, type, value, container,childtype);
				container.fadeIn('fast');
			});
		},
		createPageChildRes : function(pageNo, pageSize, type, value, container,childtype){
			container.empty();
...
//				}
		...
		createLitMeter : function(container, data){
			...
			var innerhd2 = $("<div/>").addClass("svg-panel")
				...
				.append($("<div/>").addClass("svg-pic").css({'width':'40px','height':'40px'}))
		...
		createLitFan : function(container, data){
			...
			var metricIdStr = 'instance_state';
			var status = data[metricIdStr];
			var li = status == 'CRITICAL' || status == 'CRITICAL_NOTHING' ? "<span class='ico_hardware ico_fan_red' />" : "<span class='ico_hardware ico_fan_green' />";
		...
		createLitBattery : function(container, data){
			...
			var metricIdStr = 'instance_state';
			var status = data[metricIdStr];
			var li = status == 'CRITICAL' || status == 'CRITICAL_NOTHING' ? "<span class='ico_hardware ico_battery_red' />" : "<span class='ico_hardware ico_battery_green' />";
		...
		createLitCpu : function(container, data){
			...
			var metricIdStr = 'instance_state';
			var status = data[metricIdStr];
			var li = status == 'CRITICAL' || status == 'CRITICAL_NOTHING' ? "<span class='ico_hardware ico_cpu_red' />" : "<span class='ico_hardware ico_cpu_green' />";
...
//				var status = value.availabilityStatus == 'CRITICAL' ? "<span>不可用</span>" : "<span class='ico ico_vright' style='cursor:auto;'>正常</span>";
		...
		childResourceEvent : function(instanceId, ifIndex){
			...
			that.rightDom.empty();
			oc.util.ajax({
				url : oc.resource.getUrl('portal/resource/resourceDetailInfo/getAllMetric.htm'),
				data : {
					instanceId : instanceId
				},
				successMsg:null,
		...
		createPanel : function(tdDom, tdData, tdSize){
			var width = tdData.type == 'tabs' || tdData.type == 'panel' || tdData.type == 'datagrid' ? '100%' : tdSize == 4 ? 263 : 352;
			...
			var height = (tdData.type == 'tabs' && tdData.metrics != 'none') || tdData.type == 'panel'  ? 236 : 190;
			if(tdData.type == 'tabs-full'){
				height = 280;
			}
			var titleName = (tdData.name == undefined || tdData.name == '' || tdData.name == null)
							? tdData.title : tdData.name;
			if(tdData.id == 'throughput'){
				titleName += '(最近4小时)';
			}
			var container = $("<div/>").addClass("layout-td-panelbg");
...
//					break;
				...
				return $("<label/>").css('cssText', 'min-width:25px !important').addClass(color);
			}else{
...
//					break;
		...
		createTinyTool : function(container){
			...
			if(this.cfg.resourceType == 'Host'){
				...
				ResourceCategory = 2;
			}else if(this.cfg.resourceType == 'Database' && this.cfg.categoryId == 'Oracles'){
				ResourceCategory = 3;
			}
		...
		alarmBtnEvent : function(instanceId){
			$('<div/>').dialog({
			    ...
			    onLoad:function(){
			    	oc.resource.loadScript('resource/module/alarm-management/js/alarm-resource-list.js',function(){
		...
		bizBtnEvent : function(instanceId){
			$('<div/>').dialog({
			    ...
			    width: '1108px', 
			    height: '350px',
			    href: oc.resource.getUrl('resource/module/business-service/business_service_list.html'),
			    onLoad:function(){
			    	oc.resource.loadScript('resource/module/business-service/js/business_service_list.js',function(){
			    		oc.business.service.list.open(instanceId);
			    	});
			    }
			});
		},
