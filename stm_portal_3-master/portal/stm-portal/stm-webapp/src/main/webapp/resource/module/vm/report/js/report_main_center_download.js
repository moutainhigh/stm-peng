(function($){
	oc.ns('oc.vm.main.center.download');
	var thisReportTemplateId,thisReportTemplateType,thisReportTemplateName,tempDiv;
...
//	var isEdit,isRemove;
	...
	function init(){
		...
		var queryButton = $('<a/>').attr('class','reportQueryButton').html('搜索');
		var exportExcel = $('<span/>').attr('class','fa fa-file-excel-o light_blue').attr('title','excel下载!');
		var exportWord = $('<span/>').attr('class','fa fa-file-word-o light_blue').attr('title','word下载!');
		var exportPdf = $('<span/>').attr('class','fa fa-file-pdf-o light_blue').attr('title','pdf下载!');
		var removeFile = $('<span/>').attr('class','fa fa-trash-o light_blue').attr('name','deleteReport').attr('title','删除!');
		var spanToolBar = $('<span/>').css('float','right');
		...
		var reportListFieldSet = $('<fieldset class="border-none">').css('height','85%');
		...
		var fontpageBtnbox = $('<div/>').attr('class','datagrid-pager pagination');
		var fontpageTable = '<table cellspacing="0" cellpadding="0" border="0"><tr>'+
			'<td><a href="javascript:void(0)" class="l-btn l-btn-small l-btn-plain l-btn-disabled l-btn-plain-disabled"><span class="l-btn-left l-btn-icon-left  oc-page" ><span class="l-btn-text l-btn-empty"></span><span class="l-btn-icon pagination-first" ></span></span></a></td>'+
			'<td><a href="javascript:void(0)" class="l-btn l-btn-small l-btn-plain l-btn-disabled l-btn-plain-disabled" ><span class="l-btn-left l-btn-icon-left  oc-page"><span class="l-btn-text l-btn-empty"></span><span class="l-btn-icon pagination-prev" ></span></span></a></td>'+
			'<td>第</td>'+
			'<td><input class="pagination-num" size="2" ></td>'+
			'<td><span class="paginationTotalPage"></span></td>'+
			'<td><a href="javascript:void(0)" class="l-btn l-btn-small l-btn-plain l-btn-disabled l-btn-plain-disabled" ><span class="l-btn-left l-btn-icon-left  oc-page"><span class="l-btn-text l-btn-empty"></span><span class="l-btn-icon pagination-next" ></span></span></a></td>'+
			'<td><a href="javascript:void(0)" class="l-btn l-btn-small l-btn-plain l-btn-disabled l-btn-plain-disabled" ><span class="l-btn-left l-btn-icon-left  oc-page"><span class="l-btn-text l-btn-empty"></span><span class="l-btn-icon pagination-last" ></span></span></a></td>'+
	...
	function exportFile(tempDiv,type){
	    ...
	    var iframeHtml = $("<iframe src='"+oc.resource.getUrl('portal/report/reportTemplateXmlInfo/getFileDownload.htm?xmlFileID='+
	...
	function getReportInfo(tempDiv,reportInfoDiv,topInfoDiv){
		oc.util.ajax({
			...
			success:function(data){
				...
				var topInfoDivInsideLI7 = $('<li/>').html('域: '+reportTemplateInfo.reportTemplateDomainName);
				var topInfoDivInsideLI8 = $('<li/>').html('模板创建时间: '+reportTemplateInfo.reportTemplateCreateTime);
	...
	function reportListInfoQuery(startRowTemp,rowCountTemp){
		...
		if(queryDateCheckMethod(dateStartStr,dateEndStr)){
			oc.util.ajax({
				successMsg:null,
//				url:oc.resource.getUrl('portal/vm/vmReport/getReportListByTemplateId.htm'),
	...
	function fillTempDiv(reportListData){
		for(var x=0;x<reportListData.length;x++){
			...
			tempDiv.append(reSpacing);
		}
		var reportListInfoDivTemp = reportInfoDiv.find('.reportListInfoDiv').html('');
	...
	function mainBindMethod(){
		...
		tempDiv.find('.re-spacing').on('click',function(e){
			...
			case 're-ico-position'://re-ico-position   report-bg
				...
				oc.resource.loadScript('resource/module/vm/report/js/resourcesOutlook.js',function(){
		 			oc.module.reportmanagement.vm.rol.open({reportXmlDataId:reportXmlDataId,callback:function(){
		 				switch (thisReportTemplateType) {
						case 1:
		...
		$('.oc-header-m').on('click',function(e){
			...
			case 'icon-edit ':
				oc.resource.loadScript('resource/module/vm/report/js/vm_report_template_detail_pickgrid.js', function(){
					oc.module.vm.report.template.detail.pickgrid.update(thisReportTemplateId);
				  });
				break;
			case 'ico ico-del':
					...
					buttons:[{
				    	...
				    	handler:function(){
							...
							oc.util.ajax({
								...
								success:function(data){
									oc.vm.indexpage.removeLeftMenu(thisReportTemplateId);
									
									//在菜单中去除该栏,右侧菜单变更,刷新
									switch (thisReportTemplateType) {
									case 8:
										oc.vm.indexpage.reportMainMenuClick(clickUrl,{name:"性能报告",type:8});
										break;
									case 9:
										oc.vm.indexpage.reportMainMenuClick(clickUrl,{name:"告警统计",type:9});
										break;
			...
			case 'ico ico-mark':
				...
				case 7:
					...
					break;
				case 8:
					reportTypeNameTemp = "性能报告(虚拟化)";
					break;
				case 9:
					reportTypeNameTemp = "告警报告(虚拟化)";
					break;
				case 100:
					reportTypeNameTemp = "综合报告";
					break;
				}
				var qt = thisReportTemplateName;
				oc.resource.loadScript('resource/module/vm/report/js/resourcesOutlook.js',function(){
					oc.module.reportmanagement.vm.rol
							.openPreview({
								reportTemplateId : thisReportTemplateId,
								reportName : thisReportTemplateName,
	...
	function formatReportTemplateInfo(reportTemplateInfoData){
		...
		var htmlStr = '<span class="ico ico-set locate-right" title="操作!"></span><div class="h-open-ico locate-right"  style="display:none;">';
		if(reportTemplateInfoData.edit){
			htmlStr+='<span class="icon-edit " title="编辑报表!"></span>';
		}
		//趋势报告无即时报告
		if(thisReportTemplateType!=5){
			htmlStr+='<span class="ico ico-mark" title="即时报告!"></span>';
		}
		if(reportTemplateInfoData.remove){
			htmlStr+='<span class="ico ico-del" title="删除!"></span></div>';
		}else{
			//设置产生的报表的删除
			reportInfoDiv.find('span[name=deleteReport]').css('display','none');
		}
		
//		$('.oc-header-m').find('.panel-title').html('<span>'+$('.oc-header-m').find('.panel-title').html()+'</span>'+htmlStr);
		$('.oc-header-m').find('.panel-title').html('<span>'+reportTemplateInfoData.reportTemplateName+'</span>'+htmlStr);
	...
	function formatCreateTime(templateCreateTime,type){
		...
		if(type=='day'){
			...
			return date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate()+'  '+(date.getHours()<10?'0'+String(date.getHours()):String(date.getHours()))+':'+(date.getMinutes()<10?'0'+String(date.getMinutes()):String(date.getMinutes()));	
		}else{
	...
	function formateTemplateTimeFrame(templateCycle,templateBeginTime,templateEndTime){
		if(thisReportTemplateType==5){
			//趋势报告写死
			return '每周 周一 00:00 至 周日 24:00';
		}else if(thisReportTemplateType==6){
			//分析报告写死
			return '每日 00:00 至 24:00';
		}else{
			var timeFrameStr ;
			switch (templateCycle) {
			case 1:
