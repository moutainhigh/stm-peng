...
//	var timeInterval = $('#timeInterval').combobox('getValue');
	...
	var mainDiv = $('.sessionsingle-detail-app-more-main');
	...
	oc.ui.combobox({
		placeholder: null,
		selector: mainDiv.find('.sessionsingle-detail-app-more-timeinterval'),
		data:[
			{id:'1hour',name:'最近一小时'},
			{id:'6hour',name:'最近六小时'},
			{id:'1day',name:'最近一天'},
		...
		onSelect: function(d) {
			...
			if(d.id == 'custom') {
				mainDiv.find('.sessionsingle-detail-app-more-custom-date').removeClass('hide');
				mainDiv.find('#sessionsingle_detail_app_more_search').removeClass('hide');
			} else {
				mainDiv.find('.sessionsingle-detail-app-more-custom-date').addClass('hide');
				mainDiv.find('#sessionsingle_detail_app_more_search').addClass('hide');
	...
	//type
	oc.ui.combobox({
		selector: mainDiv.find('.sessionsingle-detail-app-more-indecator'),
		data:[
		    { id: 1, name: '流量'}, 
		    { id: 2, name: '包数' }, 
		    { id: 3, name: '连接数'}/*, 
	...
	//datetimebox
	mainDiv.find('.sessionsingle-detail-app-more-custom-starttime').datetimebox();
	mainDiv.find('.sessionsingle-detail-app-more-custom-endtime').datetimebox();

	//search button
	mainDiv.find('#sessionsingle_detail_app_more_search').linkbutton('RenderLB', {
		iconCls: 'icon-search',
		onClick: function() {
			
			var t = mainDiv.find('.sessionsingle-detail-app-more-timeinterval').combobox('getValue');
			var startTime = mainDiv.find('.sessionsingle-detail-app-more-custom-starttime').datetimebox('getValue');
			var endTime = mainDiv.find('.sessionsingle-detail-app-more-custom-endtime').datetimebox('getValue');
	...
	//export button
	mainDiv.find('#sessionsingle_detail_app_more_export').linkbutton('RenderLB', {
		iconCls: 'icon-word',
		onClick: function() {
			var type = mainDiv.find('.sessionsingle-detail-app-more-indecator').combobox('getValue');
			top.location = oc.resource.getUrl("netflow/session/exportApp.htm?type="+type);
		}
	});
	...
	if(timeInterval) {
		mainDiv.find('.sessionsingle-detail-app-more-timeinterval').combobox('setValue', timeInterval);
		
		if(timeInterval == 'custom') {
			mainDiv.find('.sessionsingle-detail-app-more-custom-date').removeClass('hide');
			mainDiv.find('#sessionsingle_detail_app_more_search').removeClass('hide');

			mainDiv.find('.sessionsingle-detail-app-more-custom-starttime').datetimebox('setValue', startTime);
			mainDiv.find('.sessionsingle-detail-app-more-custom-endtime').datetimebox('setValue', endTime);
		}
	}
	
	if(type) {
		mainDiv.find('.sessionsingle-detail-app-more-indecator').combobox('setValue', type);
	}
	
	var tarDiv = mainDiv.find('.sessionsingle-app-allapps');
	oc.ui.datagrid({
		selector:tarDiv,
		pagination: true,
		sortOrder : 'desc',
