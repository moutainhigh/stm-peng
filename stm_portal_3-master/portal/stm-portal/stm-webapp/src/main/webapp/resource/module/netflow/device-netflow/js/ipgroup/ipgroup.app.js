...
//	var type = getType();
	 ...
	 * 初始化datagrid
	 */
	var	datagridDiv=$('.ipgroup-app-datagrid');
		datagridObj_netflow_ipgroup_app=oc.ui.datagrid({
	     ...
	     onLoadSuccess: function(data) {
    		oc.util.ajax({
    			...
    			success:function(data){
    				var ti = $('#device_ipgroup_timeInterval').combobox('getValue');
//    				var ti = timePerid;
    				var step = getStep(ti, data.data.timeLine);
    				var st = $('.terminal-ipg').data('startTime');
    				var et = $('.terminal-ipg').data('endTime');

    				genNetflowChart($('.ipgroup-app-hc'), 660, 135, data.data.timeLine, data.data.bos, data.data.sortColumn, data.data.yAxisName, step, ti, st, et);
    			}
    		});
	     }
	});
