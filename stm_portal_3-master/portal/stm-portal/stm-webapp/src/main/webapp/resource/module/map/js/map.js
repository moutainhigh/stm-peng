var Map = function() {
    ...
    this.convertToXY = function(t, e) {
        var a = 3600 * (Map.AREA.MAXLNG - Map.AREA.MINLNG) / this.config.width,
        n = 3600 * (Map.AREA.MAXLAT - Map.AREA.MINLAT) / this.config.height;
    ...
    this.convertToXYList = function(t) {
        ...
        return e.center = Map.calcCenter(e.range),
        e.latLng = t,
        e
    },
    this.drawCanvas = function(t) {
        for (var e = t.latLng,
        a = [], n = Map.createElement("path"), i = 0; i < e.length; i++) {
            for (var o = 0; o < e[i].length; o++) a.push((0 == o ? "M": "L") + e[i][o][0] + " " + e[i][o][1]);
            a.push("L" + e[i][0][0] + " " + e[i][0][1] + " Z")
        }
        n.setAttribute('class','topoMapPathClass');
        return n.setAttribute("autoSize", !0),
