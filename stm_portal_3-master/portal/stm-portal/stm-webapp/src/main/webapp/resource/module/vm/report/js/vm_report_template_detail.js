$(function () {
	//命名空间
	oc.ns('oc.module.vm.report.template.detail');
	//全局变量统一存储
	var globalVariableData = {};
	
	oc.module.vm.report.template.detail = {
			open : function(){
...
//							globalVariableData.curType = globalVariableData.vmPerformanceReportType;
	...
	function init(){
		...
		globalVariableData.typeData = [{id:globalVariableData.vmPerformanceReportType,name:'性能报告(虚拟化)'},
...
//		var centerContent = $('#vm_report_template_detail_main_content');
		...
		globalVariableData.comprehensiveContent = '<label style="width:60px;">章节标题：</label><div style="width:80%"><input id="directoryNameInputId" style="width:160px" type="text" maxLength=12/><span class="oc-valid-required-star">*</span>' + 
		...
		contentSettingInfo.append('<div title="章节设置"><form class="oc-form"><div id="titleContentGroupId" class="form-group"></div>' +		// col1
				'<div id="choseResourceCategorySelect" class="form-group"><label style="width:60px;">资源类型：</label><div><select id="mainResourceSelect"></select><span class="oc-valid-required-star">*</span></div></div></form>' +
				'<div class="choose-sublist"><div><div class="win-titlebg">选择资源：<input style="width:35%;" id="searchInstanceListInput" type="text" placeholder="搜索资源名称/IP/类型"/><a id="searchInstanceListButton" class="fa fa-search" style="position: relative;top: 0;left:-20px;"></a></div><div id="parentResourceInstanceGrid" style="width:620px;height:280px;float:left;"  class="tablist-bg"><div id="selectResourceInstancePickGrid" style="float:left"></div></div></div></div>' +
				'<div class="choose-sublistsecond"><div style="height:31px;line-height:31px; "><div class="win-titlebg" style="margin-bottom:0px;">选择指标：<a id="resetMetricThresholdValueButton"></a></div><div id="metricSelectGridDiv" style="height:280px;width:287px;float:left; margin-top:6px;"><div id="selectMetricGrid"></div></div></div></div><div style="clear:both; margin-top:6px;"><a class="locate-right" id="addTemplateDirectoryButton"></a></div></div>' + 
...
//							registerSortable();
				...
				//渲染资源选择和指标选择的
				globalVariableData.comboTree = oc.ui.combobox({
					  selector:$('#mainResourceSelect'),
					  width:'140px',
					  placeholder:'请选择资源类型',
					  selected:false,
//					  placeholder:false,
					  data : [{"id":"vCenter","name":"vCenter"},{"id":"cluster","name":"cluster"},{"id":"esxi","name":"esxi主机"},{"id":"vMare","name":"vMare"},{"id":"storage","name":"storage"}],
//					  value : "vCenter",
										  ...
										  success:function(data){
											  if(data.data){
												  ...
												  for(var i=0;i<data.data.length;i++){
													  var dataFlag = false;
													  for(var x=0;x<globalVariableData.curUpdateRowData.childComboValue.length;x++){
														  if(data.data[i].id != globalVariableData.curUpdateRowData.childComboValue[x].id){
															  dataFlag = true;
															  break;
														  }
													  }
				...
				globalVariableData.directory = oc.ui.datagrid({
					...
					onSelect:function(rowIndex, rowData){
						...
						if(globalVariableData.curType == globalVariableData.comprehensiveReportType){
					  	...
					  	//赋值
					  	globalVariableData.comboTree.jq.combobox('setValue',rowData.resourceTypeId);
...
//					  	globalVariableData.pickGrid.reload(globalVariableData.curUpdateRowData.childComboValue,'right');
						...
						if(globalVariableData.curType == globalVariableData.vmPerformanceReportType || (globalVariableData.curType == globalVariableData.comprehensiveReportType && globalVariableData.comprehensiveType == globalVariableData.vmPerformanceReportType)){
						...
						}else if(globalVariableData.curType == globalVariableData.vmAlarmReportType || (globalVariableData.curType == globalVariableData.comprehensiveReportType && globalVariableData.comprehensiveType == globalVariableData.vmAlarmReportType)){
							  //修改目录
							  if(globalVariableData.curUpdateRowData.isShowDetail == "true"){
								  $('#detailInfoCheckBox').attr('checked',"checked");
							  }else{
							  ...
							  if(globalVariableData.curUpdateRowData.metricValueType == 1){
								  $('#topnReportRadioType').attr('checked',"checked");
							  }else{
								  $('#topnAlarmType').attr('checked',"checked");
							  }
							  ...
							  $('.analysisReportRadioClass[value="' + globalVariableData.curUpdateRowData.metricValueType + '"]').attr('checked','checked');
						}
...
//					registerSortable();
		...
		var allTemplateType = [{id:globalVariableData.vmPerformanceReportType,name:'性能报告(虚拟化)'},
				{id:globalVariableData.vmAlarmReportType,name:'告警统计(虚拟化)'}];
	...
	function resetAllContentSetting(){
	    ...
	    globalVariableData.isShowApartLeftPickGridInfo = false;
	    globalVariableData.isSetMetricInfo = false;
		globalVariableData.comboTree.jq.combobox('setValue','');
...
//	  var parentComboValue = globalVariableData.comboTree.jq.combobox('getValue');
	  var parentComboValue = globalVariableData.pickGrid.getLeftTreeData();
	  childComboValue = globalVariableData.pickGrid.getRightTreeData();
...
function createPickGridForInstance(){
	...
	globalVariableData.pickGrid = oc.ui.picktree({
		selector:"#parentResourceInstanceGrid",
		dataType:'array',
		isInteractive:true,
		requestType:'sync',
		lUrl:oc.resource.getUrl('portal/vm/vmReport/getVmResourceByType.htm'),
		leftParam:{type:globalVariableData.currentResourceType},
		...
		onMoveSuccess:function(srcGridData,targetGridData){
			//right data
//			console.log(targetGridData);
			if(targetGridData && targetGridData.length>0){
				if(globalVariableData.metricDatagridData){
					setDatagridData(globalVariableData.dataGrid,globalVariableData.metricDatagridData);
				}else{
					oc.util.ajax({
		...
		rightCallback:{
			onAsyncSuccess:function(event, treeId, treeNode, msg){
				if(msg.data){
					var data = $.extend(true,[],msg.data);
                    // console.log(data);
//					var newData = strategyChildPicktree.rightTreeObj.transformToArray(data);
//					curRightResourceInstanceList = newData;
				}
			}
		}
	})
