function HLJRelateResourceInstance(args){
	var ctx = this;
	this.args=$.extend({
		dialog:true,
		w:600,
	...
	oc.util.ajax({
		url:oc.resource.getUrl("resource/module/topo/heiLongJiang/HLJRelateResourceInstance.html"),
...
HLJRelateResourceInstance.prototype={
	init:function(html){
		var ctx = this;
		this.root=$(html);
		if(this.args.dialog){
		...
		this.fields.grid.datagrid({
			...
			columns:[[{
				...
				field:"ip",title:"IP",width:110,sortable:true
			},{
				field:"showName",title:"名称",width:80,sortable:true
			}]]
		});
		this.fields.area.combobox({
			textField:"name",
			width:100,
			valueField:"key",
			data:this.args.areas
		});
		//初始化表格数据
		oc.util.ajax({
	...
	getAreaInfo:function(){
		var info = this.fields.area.combobox("getValue");
		var re = info.split(",");
		return {
			key:parseInt(re[0]),
			name:re[1]
	...
	select:function(mapId){
		for(var i=0;i<this.args.areas.length;i++){
			var area=this.args.areas[i];
			if(area.key.indexOf(mapId)>=0){
				this.fields.area.combobox("select",area.key);
				break;
			}
		}
	},
