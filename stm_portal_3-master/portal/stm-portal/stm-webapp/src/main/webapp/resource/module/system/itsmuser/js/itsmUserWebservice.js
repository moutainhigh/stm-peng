	...
	datagrid = oc.ui.datagrid({
		...
		onClickCell : function(rowIndex, field, value, e) {
			...
			if(field=='name'){
				...
				oc.util.ajax({
     	    		url:oc.resource.getUrl('system/itsmUser/updateSystemStartState.htm'),
...
//	     	    		successMsg:"操作成功"
	...
	function _delete() {
		...
		if(delIds==undefined || delIds.length == 0) {
			...
			oc.ui.confirm('确认删除所选择的数据？', function(){
				oc.util.ajax({
					url : oc.resource.getUrl('system/itsmUser/removeItsmSystem.htm'),
	...
	function _open(type) {
		...
		oc.resource.loadScript(
			'resource/module/system/itsmuser/js/itsmSystemDetail.js', function() {
				oc.module.itsmsystem.detail.open({
					type : type,
					id : id,
					callback : function() {
						datagrid.reLoad();
