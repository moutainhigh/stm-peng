$(function(){
	...
	oc.ns('oc.customprofile.strategy.detail');
	...
	function initDetail(){
					  ...
					  addChildStrategy.find('.oc-toolbar').append($('<a/>').linkbutton('RenderLB',{
						  ...
						  onClick:function(){
							  if(addChildForm.validate()){
								  ...
								  addParameter.profileTypeMapping = 1;
								  
								  addChildProfile(addParameter,addChildStrategyDialog);
		...
		if(allData.children != null && allData.children.length > 0){
			...
			for(var i = 0 ; i < resourceList.length ; i++){
				...
				if(isAllowUpdateAllProfile){
					childStrategyTitle += '<span resourceId="' + resourceIdList[i] + '" style=" margin-top:1;float: right; position: relative;right: 5px;top:3px;" class="fa fa-close">&nbsp;</span>' + 
						'<span class="l-btn-icon fa fa-plus" resourceId="' + resourceIdList[i] + '" style=" top: 2px;float: right; position: relative;right: 5px;"></span>';
				}
				...
				var navSubList = oc.ui.navsublist({
					...
					addRowed:function(li,data){
						...
						if(isAllowUpdateAllProfile){
							li.append('<span class="fa fa-close fa fa-times-circle oc-icomargin">&nbsp;</span>');
						}
							
					},
					click:function(href,data,e){   
						if($(e.target).hasClass("fa-close")){//modify by sunhailiang on 20170619 
							//删除子资源
			...
			if(isAllowUpdateAllProfile){
				...
				//删除同类型的所有子策略
				$('#resource_children_strategy_id').find('.personalizeChildProfileTitle').find('.fa-close').on('click',function(e){
					
					e.stopPropagation();
					
					var deleteResourceId = $(e.target).attr('resourceId');
					
					oc.ui.confirm('确认删除' + $(e.target).parent().text() + '的所有子策略吗?',function(){
						
						  var profileIds = new Array();
						
						  for(var j = 0 ; j < allData.children.length ; j++){
							
								if(allData.children[j].profileInfo.resourceId == deleteResourceId){
									profileIds.push(allData.children[j].profileInfo.profileId);
								}
						  ...
						  oc.util.ajax({
							  successMsg:null,
							  url: oc.resource.getUrl('portal/strategydetail/removeProfileByIdList.htm'),
							  data:{profileIds:profileIds.join(',')},
							  success:function(data){
	...
	function addChildProfile(addParameter,dialog){
		...
		if(curProfileData.children){
			...
			for(var i = 0 ; i < curProfileData.children.length ; i ++){
				if(addParameter.profileName == curProfileData.children[i].profileInfo.profileName){
					//重复
					alert('子策略名重复!');
					return;
				}
	...
	function initAccrodion(isChildStrategy,strategyIndex){
		...
		if(isChildStrategy){
			...
			var nowDomainId = curProfileDomainId;
			...
			resourceSelectDiv.append('<form class="oc-form col1">'+
					...
					'<input class="left" type="text" id="searchResourceOrIpInputForChildProfile" placeholder="搜索资源名称|IP"/><a id="h4ForChildProfile"><span class="fa fa-search"></span>查找</a>'+
					'</div></form>');
			...
			'<input class="left" style="margin-right:5px;" type="text" id="searchChildResourceName" placeholder="搜索资源名称"/><a id="queryChildResourceName" style="text-align:right">查找</a>'+
			'</div>');
...
//                		$('#childResourceSelectLeftParentGrid').find('.selectChildResourceHref[parentId="' + row.resourceId + '"]').trigger("click",["unCheck"]);
			...
			childRightCfg = {
					...
					columns:[[
			                    ...
			                    {field:'resourceName',title:'子资源名称',width:180,ellipsis:true},
			                    {field:'strategyName',title:'当前策略',width:100,ellipsis:true,
			                    	formatter: function(value,row,index){
			                    		if(!value){
			                    			return "N/A";
			                    		}else{
			...
			cfg = {
                ...
                onCheckAll:function(rows){
            		...
            		var domainIdParam = curProfileDomainId;
	    	...
	    	alarmRulesDiv.panel({
	    		...
	    		onLoad:function(){
	    			oc.resource.loadScript('resource/module/resource-management/receiveAlarmQuery/js/alarmRules.js',function(){
	    				if(isAllowUpdateAllProfile){
	    					oc.resource.alarmrules.open(singleStrategyInfo.profileInfo.profileId,'model_profile',curProfileDomainId,1);
    					}else{
    						oc.resource.alarmrules.openDataCheck(singleStrategyInfo.profileInfo.profileId,'model_profile',curProfileDomainId,1);
    					}
	    			});
	    		}
	    	});
	    			...
	    			$("#openTimeLineButtonId").linkbutton('RenderLB',{
	    				...
	    				onClick:function(e){
	    					oc.resource.loadScript('resource/module/resource-management/timeline/js/timeline.js',function(){
	    						oc.timeline.dialog.open(singleStrategyInfo.profileInfo.profileId,singleStrategyInfo.profileInfo.resourceId,curProfileData.profileInfo.createUser);
	    					}); 
	    				}
		...
		if(isChildStrategy){
			...
			if(isNetWorkInterface){
				...
				var col = childProfileRightGrid.selector.datagrid('getColumnOption', 'resourceName');
				col.width = 90;
				childProfileRightGrid.selector.datagrid();
			}
...
//		  }
	...
	oc.customprofile.strategy.detail = {
...
//			}
	...
	function updateProfileTotalInfo(updateInfo,nextShowSelectIndex,isChildStrategy,strategyIndex){
		...
		oc.util.ajax({
			  ...
			  data:{mainProfileId:nowStrategyId,profileInfo:updateInfo.modifyInfo,profileInfoVo:updateInfo.modifyResourceInfo,
				  metricString:updateInfo.modifyMetricInfo,updateTime:nowTimestamp},
			  success:function(data){
				  ...
				  if(data.data){
					  ...
					  if(updateInfo.modifyAlarmInfo){
	    		  	  ...
	    		  	  //有名称修改则刷新策略列表
	    		  	  if(!curIsQuickSelectStrategy && updateInfo.modifyInfo && updateInfo.modifyInfo.profileName && !isChildStrategy){
