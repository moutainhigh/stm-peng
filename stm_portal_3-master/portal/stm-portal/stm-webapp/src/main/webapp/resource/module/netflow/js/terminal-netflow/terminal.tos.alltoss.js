...
//	var timeInterval = $('#timeInterval').combobox('getValue');
	var mainDiv = $('.terminalsingle-detail-tos-more-main');
	...
	oc.ui.combobox({
		placeholder: null,
		selector: mainDiv.find('.terminalsingle-detail-tos-more-timeinterval'),
		data:[
			{id:'1hour',name:'最近一小时'},
			{id:'6hour',name:'最近六小时'},
			{id:'1day',name:'最近一天'},
		...
		onSelect: function(d) {
			...
			if(d.id == 'custom') {
				mainDiv.find('.terminalsingle-detail-tos-more-custom-date').removeClass('hide');
				mainDiv.find('#terminalsingle_detail_tos_more_search').removeClass('hide');
			} else {
				mainDiv.find('.terminalsingle-detail-tos-more-custom-date').addClass('hide');
				mainDiv.find('#terminalsingle_detail_tos_more_search').addClass('hide');
	...
	//type
	oc.ui.combobox({
		selector: mainDiv.find('.terminalsingle-detail-tos-more-indecator'),
		data:[
		    { id: 1, name: '流量'}, 
		    { id: 2, name: '包数' }, 
		    { id: 3, name: '连接数'}/*, 
		...
		placeholder: null,
		onSelect: function(d) {
			changeIndicatorDetail(tarDiv, d.id, 'tos', 2, false);
		}
	});

	//datetimebox
	mainDiv.find('.terminalsingle-detail-tos-more-custom-starttime').datetimebox();
	mainDiv.find('.terminalsingle-detail-tos-more-custom-endtime').datetimebox();

	//search button
	mainDiv.find('#terminalsingle_detail_tos_more_search').linkbutton('RenderLB', {
		iconCls: 'icon-search',
		onClick: function() {
			
			var t = mainDiv.find('.terminalsingle-detail-tos-more-timeinterval').combobox('getValue');
			var startTime = mainDiv.find('.terminalsingle-detail-tos-more-custom-starttime').datetimebox('getValue');
			var endTime = mainDiv.find('.terminalsingle-detail-tos-more-custom-endtime').datetimebox('getValue');
	...
	//export button
	mainDiv.find('#terminalsingle_detail_tos_more_export').linkbutton('RenderLB', {
		iconCls: 'icon-word',
		onClick: function() {
			var type = mainDiv.find('.terminalsingle-detail-tos-more-indecator').combobox('getValue');
			top.location = oc.resource.getUrl("netflow/terminal/exportTos.htm?type="+type);
		}
	});
	...
	if(timeInterval) {
		mainDiv.find('.terminalsingle-detail-tos-more-timeinterval').combobox('setValue', timeInterval);
		
		if(timeInterval == 'custom') {
			mainDiv.find('.terminalsingle-detail-tos-more-custom-date').removeClass('hide');
			mainDiv.find('#terminalsingle_detail_tos_more_search').removeClass('hide');

			mainDiv.find('.terminalsingle-detail-tos-more-custom-starttime').datetimebox('setValue', startTime);
			mainDiv.find('.terminalsingle-detail-tos-more-custom-endtime').datetimebox('setValue', endTime);
		}
	}
	
	if(type) {
		mainDiv.find('.terminalsingle-detail-tos-more-indecator').combobox('setValue', type);
	}
	
	var tarDiv = $('.terminalsingle-tos-alltos');
	oc.ui.datagrid({
		selector:tarDiv,
		pagination: true,
		pageSize: 20,
		...
		columns:[[
	         ...
	         {field:'tos_name',title:'tos名称',sortable:false,width:'10%'},
