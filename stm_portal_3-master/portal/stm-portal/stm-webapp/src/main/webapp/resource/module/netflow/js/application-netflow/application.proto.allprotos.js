...
//	endTime=timeObj.endTime;
	...
	var mainDiv = $('.appsingle-detail-proto-more-main');

	//timeinterval
	oc.ui.combobox({
		placeholder: null,
		selector: mainDiv.find('.appsingle-detail-proto-more-timeinterval'),
		data:[
			{id:'1hour',name:'最近一小时'},
			{id:'6hour',name:'最近六小时'},
			{id:'1day',name:'最近一天'},
		...
		onSelect: function(d) {
			...
			if(d.id == 'custom') {
				mainDiv.find('.appsingle-detail-proto-more-custom-date').removeClass('hide');
				mainDiv.find('#appsingle_detail_proto_more_search').removeClass('hide');
			} else {
				mainDiv.find('.appsingle-detail-proto-more-custom-date').addClass('hide');
				mainDiv.find('#appsingle_detail_proto_more_search').addClass('hide');
	...
	//type
	oc.ui.combobox({
		selector: mainDiv.find('.appsingle-detail-proto-more-indecator'),
		data:[
		    { id: 1, name: '流量'}, 
		    { id: 2, name: '包数' }, 
		    { id: 3, name: '连接数'}/*, 
	...
	//datetimebox
	mainDiv.find('.appsingle-detail-proto-more-custom-starttime').datetimebox();
	mainDiv.find('.appsingle-detail-proto-more-custom-endtime').datetimebox();

	//search button
	mainDiv.find('#appsingle_detail_proto_more_search').linkbutton('RenderLB', {
		iconCls: 'icon-search',
		onClick: function() {
			
			var t = mainDiv.find('.appsingle-detail-proto-more-timeinterval').combobox('getValue');
			var startTime = mainDiv.find('.appsingle-detail-proto-more-custom-starttime').datetimebox('getValue');
			var endTime = mainDiv.find('.appsingle-detail-proto-more-custom-endtime').datetimebox('getValue');
	...
	//export button
	mainDiv.find('#appsingle_detail_proto_more_export').linkbutton('RenderLB', {
		iconCls: 'icon-word',
		onClick: function() {
			var type = mainDiv.find('.appsingle-detail-proto-more-indecator').combobox('getValue');
			top.location = oc.resource.getUrl("netflow/application/exportProto.htm?type="+type);
		}
	});
	...
	if(timeInterval) {
		mainDiv.find('.appsingle-detail-proto-more-timeinterval').combobox('setValue', timeInterval);
		
		if(timeInterval == 'custom') {
			mainDiv.find('.appsingle-detail-proto-more-custom-date').removeClass('hide');
			mainDiv.find('#appsingle_detail_proto_more_search').removeClass('hide');

			mainDiv.find('.appsingle-detail-proto-more-custom-starttime').datetimebox('setValue', startTime);
			mainDiv.find('.appsingle-detail-proto-more-custom-endtime').datetimebox('setValue', endTime);
		}
	}
	
	if(type) {
		mainDiv.find('.appsingle-detail-proto-more-indecator').combobox('setValue', type);
	}
	
	var tarDiv = $('.appsingle-proto-allprotos');
	oc.ui.datagrid({
		selector:tarDiv,
		width:500,
		queryParams :{
		...
		columns:[[
	         ...
	         {field:'proto_name',title:'协议名称',sortable:false,width:'9%'},
