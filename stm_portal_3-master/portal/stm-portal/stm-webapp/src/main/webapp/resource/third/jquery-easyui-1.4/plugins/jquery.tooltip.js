...
//			_c.onPosition.call(_a, _e.left, _e.top);
		...
		function _ps(my,of){
			of=$(of);
			var offset=20,myP;
			_d.css('zIndex',(_c.zIndex != undefined ? _c.zIndex : ($.fn.window ? $.fn.window.defaults.zIndex++ : "")));
			if(of.parent().is('.combo')){
				myP='left+30 center';
			}else{
				myP='left+18 center';
			}
			my.position({my:myP,at:'right center',of:of,collision:'none'});
			if((my.position().left+my.width()+offset)>$('body').width()){
				my.position({my:'left top',at:'left bottom+1',of:of,collision:'none'});
			}
		}
	...
	function _17(_1d, _1e) {
		...
		var cc = typeof _20.content == "function" ? _20.content.call(_1d)
				: _20.content;
	...
	$.fn.tooltip = function(_25, _26) {
		...
		return this.each(function() {
			...
			_3(this);
			_17(this);
			oc.util.whileOne(window,'resize' ,150,(function(jq){
				jq=$(jq);
				return function(){
					jq.tooltip('reposition');
				};
			})(this));
		});
	};
