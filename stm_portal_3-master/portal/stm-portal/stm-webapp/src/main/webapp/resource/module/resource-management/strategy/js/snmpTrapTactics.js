(function($) {
	var mainDiv = $('#oc_module_resource_management_snmptraptactics').attr('id',
			oc.util.generateId()).panel({
		fit : true,
		isOcAutoWidth : true
	}), 
	datagridDiv = mainDiv.find('.oc_module_resource_management_snmptraptactics_datagrid').first(); 
	var datagrid = null;
	...
	datagrid = oc.ui.datagrid({
		...
		pageSize:$.cookie('pageSize_snmpTrapTactics')==null ? 15 : $.cookie('pageSize_snmpTrapTactics'),
		url : oc.resource.getUrl('resourcelog/snmptrap/queryStrategy.htm'),
		queryParams:{typeId:2,creatorId:userId},
		octoolbar : {
			right : toolbar
		},
		columns : [ [
	...
	if(paginationObject){
		paginationObject.pagination({
			onChangePageSize:function(pageSize){
				var before_cookie = $.cookie('pageSize_snmpTrapTactics');
				$.cookie('pageSize_snmpTrapTactics',pageSize);
			},
		});
	}
	...
	function _copyStrategy() {
		...
		oc.resource.loadScript(
			'resource/module/resource-management/strategy/js/snmptrapCopy.js', function() {
				oc.snmptrap.strategy.copy.open({
					id : id[0],
					callback : function() {	
						datagrid.reLoad();
					}
	...
	function _delStrategy() {
		...
		if(delIds==undefined || delIds.length == 0) {
			...
			oc.ui.confirm('确认删除所选择的数据？', function(){
				oc.util.ajax({
					url : oc.resource.getUrl('resourcelog/snmptrap/removeStrategy.htm'),
	...
	function _open(type, name) {
		...
		oc.resource.loadScript(
			'resource/module/resource-management/strategy/js/snmpTrapTacticsDetail.js', function() {
				oc.snmptraptacticsdetail.open({
					type : type,
					name : name,
					id : id,
					callback : function() {
