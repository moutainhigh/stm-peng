...
//	var timeInterval = $('#timeInterval').combobox('getValue');
	...
	var mainDiv = $('.ipgroup-detail-tos-more-main');
	...
	//timeInterval init
	oc.ui.combobox({
		selector: mainDiv.find('.ipgroup-detail-tos-more-timeinterval'),
		data : [ {
			id : '1hour',
			name : '最近一小时'
		}, {
		...
		onSelect: function(d) {
			...
			if(d.id == 'custom') {
				mainDiv.find('.ipgroup-detail-tos-more-custom-date').removeClass('hide');
				mainDiv.find('#ipgroup_detail_tos_more_search').removeClass('hide');
			} else {
				mainDiv.find('.ipgroup-detail-tos-more-custom-date').addClass('hide');
				mainDiv.find('#ipgroup_detail_tos_more_search').addClass('hide');
				
				var startTime = mainDiv.find('.ipgroup-detail-tos-more-custom-starttime').datetimebox('getValue');
				var endTime = mainDiv.find('.ipgroup-detail-tos-more-custom-endtime').datetimebox('getValue');
				...
				ipgTosAllDatagridDiv.datagrid('load', data);
			}			
		}
	});
	...
	//type init
	oc.ui.combobox({
		selector : mainDiv.find('.ipgroup-detail-tos-more-indecator'),
		data:[
		    { id: 1, name: '流量'}, 
		    { id: 2, name: '包数' }, 
		    { id: 3, name: '连接数'}/*, 
		...
		selected: '1',
		onSelect : function(d) {
			changeIndicatorDetail(ipgTosAllDatagridDiv, d.id, 'tos', 1, false);
		}
	});
	
	//datetimebox init
	mainDiv.find('.ipgroup-detail-tos-more-custom-starttime').datetimebox();
	mainDiv.find('.ipgroup-detail-tos-more-custom-endtime').datetimebox();
	
	//search button init
	mainDiv.find('#ipgroup_detail_tos_more_search').linkbutton('RenderLB', {
		iconCls: 'icon-search',
		onClick: function() {
			var startTime = mainDiv.find('.ipgroup-detail-tos-more-custom-starttime').datetimebox('getValue');
			var endTime = mainDiv.find('.ipgroup-detail-tos-more-custom-endtime').datetimebox('getValue');
			var t = mainDiv.find('.ipgroup-detail-tos-more-timeinterval').combobox('getValue');
	...
	//export button
	mainDiv.find('#ipgroup_detail_tos_more_export').linkbutton('RenderLB', {
		iconCls: 'icon-word',
		onClick: function() {
			var type = mainDiv.find('.ipgroup-detail-tos-more-indecator').combobox('getValue');
			top.location = oc.resource.getUrl("netflow/ipgroup/tos/exportTos.htm?type="+type);
		}
	});
	...
	if(timeInterval) {
		mainDiv.find('.ipgroup-detail-tos-more-timeinterval').combobox('setValue', timeInterval);
		
		if(timeInterval == 'custom') {
			mainDiv.find('.ipgroup-detail-tos-more-custom-date').removeClass('hide');
			mainDiv.find('#ipgroup_detail_tos_more_search').removeClass('hide');
			
			mainDiv.find('.ipgroup-detail-tos-more-custom-starttime').datetimebox('setValue', startTime);
			mainDiv.find('.ipgroup-detail-tos-more-custom-endtime').datetimebox('setValue', endTime);
		}
	}
	
	if(type) {
		mainDiv.find('.ipgroup-detail-tos-more-indecator').combobox('setValue', type);
	}
	...
	var ipgTosAllDatagridDiv= mainDiv.find('.ipgroup-tos-alltoss'),
		
		datagridObj_netflow_tos=oc.ui.datagrid({
			selector:ipgTosAllDatagridDiv,
			...
			columns:[[
	         {field:'name',title:'协议名称',sortable:false,width:'12%',align:'center'},
	...
	changeIndicatorDetail(ipgTosAllDatagridDiv, type, 'tos', 1, false);
