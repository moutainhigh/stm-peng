$(function () {
	...
	function initFrom(cfg,operatingDialog){
		formDiv=operatingDialog.find('#snmpTrapOperating');
		...
		form=oc.ui.form({
			...
			combobox:[{
				...
				url : oc.resource.getUrl('resourcelog/snmptrap/querySetStrategy.htm?typeId=2&domainIds='+cfg.row.domainId+"&creatorId="+userId),
				placeholder:null,
	...
	function open(cfg){
		...
		operatingDialog=operatingDialogDiv.dialog({
		    ...
		    buttons:[{
		    	...
		    	handler:function(){
		    		oc.util.ajax({
						url : oc.resource.getUrl('resourcelog/snmptrap/updateResourceStrategy.htm'),
						data : form.val(),
						async:false,
		    ...
		    href: oc.resource.getUrl('resource/module/resource-management/dailyRecord/snmptrapOperating.html'),
		    onLoad:function(){
