(function($){
	 ...
	 * 初始化datagrid
	 */
	var datagridDiv=curTab.find('.interface-tos-datagrid'),
		...
		datagridObj_netflow_tos=oc.ui.datagrid({
	     ...
	     onLoadSuccess: function(data) {
    		oc.util.ajax({
    			url:oc.resource.getUrl('netflow/if/tos/getIfTosChartData.htm'),
    			data: data.paramBo,
    			...
    			success:function(data){
    				...
    				genNetflowChart(curTab.find('.interface-tos-hc'), 660, 135, data.data.timeLine, data.data.bos, data.data.unit, data.data.yAxisName, step, ti, st, et);
    			}
    		});
	     }
	});
