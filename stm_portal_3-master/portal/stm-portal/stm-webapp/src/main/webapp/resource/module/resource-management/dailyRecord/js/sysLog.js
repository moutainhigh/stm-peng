(function($) {
	var datagrid = null;
	var mainDiv = $('#oc_module_resource_management_syslog').attr('id',
			oc.util.generateId()).panel({
		fit : true,
		isOcAutoWidth : true
	}), 
	datagridDiv = mainDiv.find('.oc_module_resource_management_syslog_datagrid').first(), 
...
//			domainData.unshift({id:0,name:"all"});//添加到数组的开头
	...
	var userTypes = oc.util.getDictObj('user_type'), toolbar = [ {
		...
		onClick : function() {
    		...
    		oc.util.ajax({
				url : oc.resource.getUrl('portal/syslog/updateMonitorState.htm'),
	...
	datagrid = oc.ui.datagrid({
		...
		url : oc.resource.getUrl('portal/syslog/querySyslogList.htm'),
		queryParams:{typeId:1},//syslog类型1
		octoolbar : {
			right : toolbar,
		...
		columns : [ [
				...
				},
				{
					...
					width : 30
//					formatter:function(value,row,index){
//						return oc.index.getCategoryMapper()[value];
//					}
				},
				{
					field : 'curDateCount',
...
//							return date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate()+' '+(date.getHours()==0?'00':String(date.getHours()))+':'+(date.getMinutes()==0?'00':String(date.getMinutes()))+':'+(date.getSeconds()==0?'00':String(date.getSeconds()));
		...
		onClickCell : function(rowIndex, field, value, e) {
			...
			if(field=="operating"){
				oc.resource.loadScript('resource/module/resource-management/dailyRecord/js/sysLogOperating.js', function() {
			...
			//当日产生数量
				...
				oc.resource.loadScript('resource/module/resource-management/dailyRecord/js/sysLogDayProduced.js', function() {
					oc.syslog.dayproduced.dialog.open(row,"dayProduced");
				});
			}
			//总计
			else if(field=="allCount"){
				oc.resource.loadScript('resource/module/resource-management/dailyRecord/js/sysLogDayProduced.js', function() {
					oc.syslog.dayproduced.dialog.open(row,"total");
				});
			}
		}
	});
