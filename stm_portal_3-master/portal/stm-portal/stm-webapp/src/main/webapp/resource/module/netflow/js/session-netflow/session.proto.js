...
//	var endTime = $('.device-session').data('endTime');
	...
	var tarDiv = tab.find('.session-proto-datagrid');
	datagridObj_netflow_session_proto=oc.ui.datagrid({
		selector: tarDiv,
		queryParams: {
			starttime: startTime,
			endtime: endTime,
...
//	    	 }
	    	    ...
	    	    tab.find('.session-proto-hc').highcharts({
	    	        chart: {
	    	            type: 'pie',
...
function showSessionProtos(){
	var rows = $(".session-proto-datagrid-selector").datagrid("getRows");
	if(rows != null && 0!=rows &&''!= rows){
	 $('<div class="session-proto-allproto"></div>').dialog({    
		 	title: "协议",
		    width: 1200,
		    height: 605, 
		    top: '5%',
		    ...
		    position:'center',
		    draggable:false,
		    href: oc.resource.getUrl("resource/module/netflow/session-netflow/session_proto.html"),    
		    modal: true
		});
	}
}