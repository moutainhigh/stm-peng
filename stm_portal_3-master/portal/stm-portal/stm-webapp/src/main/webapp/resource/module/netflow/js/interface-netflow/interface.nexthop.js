(function($){
	 ...
	 * 初始化datagrid
	 */
	var datagridDiv=curTab.find('.interface-nexthop-datagrid'),
		...
		datagridObj_netflow_nexthop=oc.ui.datagrid({
			...
			columns:[[
	         {field:'nextHop',title:'下一跳IP',sortable:false,width:'20%',align:'center'},
	     ...
	     onLoadSuccess: function(data) {
    		oc.util.ajax({
    			url:oc.resource.getUrl('netflow/if/nexthop/getIfNextHopChartData.htm'),
    			data: data.paramBo,
    			...
    			success:function(data){
    				...
    				genNetflowChart(curTab.find('.interface-nexthop-hc'), 660, 135, data.data.timeLine, data.data.bos, data.data.unit, data.data.yAxisName, step, ti, st, et);
    			}
    		});
	     }
	});
