...
//		$(".edui-container .edui-toolbar").hide();
	...
	var alarmRuleContentTable = $('<table/>').css('height','98%').css('width','100%').addClass("tab-border").attr('cellspacing','0').attr('cellpadding','0').attr('border','0');//addClass('table-pop')
	...
	var alarmRuleContentTableTr1Td1 = $('<td/>').html('告警方式:').addClass('tab-l-tittle').addClass('td-bgcolor');//.attr('style','line-height:30px;');
	...
	var alarmRuleContentTableTr2Td1 = $('<td/>').html('接收人员:').addClass('tab-l-tittle').addClass('td-bgcolor');
	var alarmRuleContentTableTr2Td2 = $('<td/>').attr('valign','middle').html(
			...
			'<div style="height:65%;"><textarea disabled = "true" name="textareaPerson" style="resize: none;margin: 0px 2px; height: 50px; width: 60%;"></textarea><a name="addPerson" title="选择告警接收人!" class="fa fa-hand-o-up ico-change" style="float: none;"></a></div>');
	
	
	var alarmRuleContentTableTr3Td1 = $('<td/>').html('告警规则:').addClass('tab-l-tittle').addClass('td-bgcolor').attr('valign','middle');
...
//	}
	...
	var alarmRuleContentTableTr4Td1=$('<td/>').html('短信内容:').addClass('tab-l-tittle').addClass('td-bgcolor').addClass('notifycontent');
...
+'</div>'
	...
	var alarmRuleContentTableTr5Td1 = $('<td/>').html('发送条件:').addClass('tab-l-tittle').addClass('td-bgcolor');
	...
	var alarmRuleContentTableTr5Td2 = $('<td/>').attr('valign','middle').html(
			...
			'<label>同一条告警间隔&nbsp;</label><input type="text" validType="[\'int\',\'maxLength[6]\']"  name="continusCount" class="continusCount" style="vertical-align:middle;margin-right:5px;">'+
			'<select name="localDatatest" class="localData" ></select>'+
			//增加最多发送次数输入框 dfw 20161227
			'<label>&nbsp;连续发送，<span id="sendTimes">最多发送&nbsp;<input type="text" validType="[\'int\',\'maxLength[6]\']"  name="sendTimes" class="sendTimes">&nbsp;次，</span>直到系统被恢复</label>'
	);
	var alarmRuleContentTableTr6 = $('<tr/>').css('height','30%');
	var alarmRuleContentTableTr6Td1 = $('<td/>').html('发送告警时间:').addClass('tab-l-tittle').addClass('td-bgcolor');
	...
	var alarmRuleContentTableTr6Td2DivSmsSendTimeSet = $('<div/>').css('height','20px').html('<input type="radio" checked=true value=1 name="sendAlarmSmsTimeSet">24小时*7天&nbsp;&nbsp;</input><input type="radio" value=2 name="sendAlarmSmsTimeSet">自定义</input>');
	var alarmRuleContentTableTr6Td2DivEmailSendTimeSet = $('<div/>').css('height','20px').html('<input type="radio" checked=true value=1 name="sendAlarmEmailTimeSet">24小时*7天&nbsp;&nbsp;</input><input type="radio" value=2 name="sendAlarmEmailTimeSet">自定义</input>');
	var alarmRuleContentTableTr6Td2DivAlertSendTimeSet = $('<div/>').css('height','20px').html('<input type="radio" checked=true value=1 name="sendAlarmAlertTimeSet">24小时*7天&nbsp;&nbsp;</input><input type="radio" disabled="true" value=2 name="sendAlarmAlertTimeSet">自定义</input>');
	var alarmRuleContentTableTr6Td2DivSmsRandomTime = $('<div/>').css('width','60%').addClass("td-no-border");
...
//	
	...
	function formatForm(form){
		...
		/*	form.find("input[name='sendCondition']").on('click',function(){
				var target=$(this);
			...
			form.find('.ico-change').on('click',function(e){
				if(form.find('.ico-change').attr('name')=='addPerson'){
					var addPerson = $('<div/>');
						...
						success:function(data){
							...
							personListDiv.dialog({
							 ...
							 buttons:[{
								...
								handler:function(){
							var checkedRow = personDatagrid.selector.datagrid('getChecked');
							
							//选择的发送方式
							    		switch (currentSendWay) {
										case 1:
											getChooseUser(personListDiv,sendContentUser,form,checkedRow);
											break;
										case 2:
											getChooseUser(personListDiv,sendContentUser,form,checkedRow);
											break;
					...
					columns:[[
				     ...
				     for(var i=0;i<personPicked.length;i++){
				     $.each(rowData,function(index, value){
				     if(personPicked[i]==rowData[index].id){
				     $('#personDatagrid').datagrid("checkRow", index);
				     }
				     });
				     }
				     }
				    ...
				    //modify by sunhailiang on 20170606	 
					var userData = new Array(); 
					for(var i = 0;i<personList.length;i++){
					  var id = personList[i].id + "";
					  if($.inArray(id,alarmPersonIds) == -1){ 
						  userData.push(personList[i]);
					  }
					} 
					
					var localData = {"data":{"total":0,"rows":userData},"code":200};
				
					personDatagrid.selector.datagrid('loadData',localData);	
							}
...
//				}else 
				if($(this).attr('name')=='delAlarmTimeSetWeek'){
...
//	}
	function fillValueToPage(form,sendContent){
		if("checked"==sendContent.isSmsAlarm){
			form.find('input[name="isSmsAlarm"]').prop('checked',true);
		}
		if("checked"==sendContent.sendTimeSet || true==sendContent.sendTimeSet){
			form.find('input[name="sendCondition"]').prop('checked',true);
		}
...
//				}
		...
		//其余部分数据保存
		var isSmsAlarmTemp = form.find('input[name="isSmsAlarm"]').prop('checked');
		var checkedLevelArr = new Array();
		form.find('input[name="level"]').each(function(){
			var checkedLevelTemp = $(this);
			if(checkedLevelTemp.prop('checked')){
				checkedLevelArr.push(checkedLevelTemp.prop('value'));
			}
		})
		
		var sendTimeSet = form.find('input[name="sendCondition"]').prop('checked');
		...
		var type=form.find('[name="alarmSendWay"].active').prop('type');
		if(type==1){
		...
		//修改sendContent值
		sendContent.isSmsAlarm = isSmsAlarmTemp==true?'checked':'';
	...
	function getChooseUser(dialog,sendContent,form,checkeds){
	
		var receiveUserTemp = new Array();
		var userNameTextarea = sendContent.receiveUserTextarea = "";
		
		for(var i=0;i<checkeds.length;i++){
		receiveUserTemp.push(checkeds[i].id);
		if(""==userNameTextarea){
					userNameTextarea = ' < '+checkeds[i].account+' > ';
				}else{
					userNameTextarea += ' ,  < '+checkeds[i].account+' > ';
				}
		}
	...
	function formReset(form){
		...
		if(type==3){
			...
			form.find(".notifycontent").html('邮件内容:');
			form.find("table tr").eq(3).attr('style','height:8%');
		}else{
			form.find(".notifycontent").html('短信内容:');
			form.find("table tr").eq(3).attr('style','height:8%');
		}
