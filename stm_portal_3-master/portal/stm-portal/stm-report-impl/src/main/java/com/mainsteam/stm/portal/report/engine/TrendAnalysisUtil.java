...
public class TrendAnalysisUtil {
	 ...
	 */
	public TrendAnalysisUtil(DataPoint data[]) {
		pn = 0;
		xy = new String[2];
		listX = new ArrayList();
		listY = new ArrayList();
...
//			System.out.println(xy[1]);
	...
	public DataPoint getNextDataPoint(){
		float pointX=this.getDataPointCount()+1;
		float pointY=this.getA1()*pointX+ this.getA0();
		return new DataPoint(pointX,pointY);
	}
	 ...
	 * @return
	 */
	public DataPoint[] getBeginEndPoint(List<DataPoint> dataPoints,ReportCycleEnum cycle){
		...
		int pointCount=7;
		if(cycle.equals(ReportCycleEnum.MONTH)){
			pointCount=31;
		}
		
		dataPointArr[0]=this.getNextDataPoint();
		for(int i=1;i<pointCount;i++){
			DataPoint dataPoint=this.getNextDataPoint();
			this.addDataPoint(dataPoint);
		}
		dataPointArr[1]=this.getNextDataPoint();
