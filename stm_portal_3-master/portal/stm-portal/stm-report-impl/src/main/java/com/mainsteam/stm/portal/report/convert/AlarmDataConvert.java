...
public class AlarmDataConvert extends BaseDataConvert{
	...
	//告警分布，级别分布，状态分布
	public AlarmDataConvert(ReportTemplate reportTemplate,ReportTemplateDirectory reportTemplateDirectory){
	...
	public ReportDirectory getReportDirectory(){
		...
		List<Long> queryInstanceId = new ArrayList<Long>();
		for(Long instanceId : this.instanceIDes){
			try {
				ResourceInstance instance = resourceInstanceService.getResourceInstance(instanceId);
				if(instance != null && instance.getParentId() <= 0){
					//主资源
					queryInstanceId.add(instanceId);
					List<ResourceInstance> childResourceInstance = instance.getChildren();
					if(childResourceInstance != null){
						for(ResourceInstance childInstance : childResourceInstance){
							queryInstanceId.add(childInstance.getId());
						}
					}
				}else if(instance != null && instance.getParentId() > 0){
					//子资源
					queryInstanceId.add(instanceId);
				}
		...
		query.setInstanceIDes(queryInstanceId);
	...
	public Table getSumTable(Collection<InstanceAlarmEventReportData> alarmDatas){
		...
		if(alarmDatas==null || alarmDatas.size()<=0){
			...
			for(ReportTemplateDirectoryInstance instance:instanceList){
				...
				try {
					resourceInstance = resourceInstanceService.getResourceInstance(instance.getInstanceId());
				} catch (InstancelibException e) {
					logger.error(e.getMessage(),e);
				}
				...
				int childSerious = 0;
				int childWarn = 0;
				int childNotRecover = 0;
				int childRecover = 0;
				
				if(resourceInstance != null && resourceInstance.getParentId() <= 0){
					//主资源
					if(resourceInstance.getChildren() != null){
						for(ResourceInstance childResourceInstance : resourceInstance.getChildren()){
							InstanceAlarmEventReportData alarmData=alarmDataMap.get(childResourceInstance.getId());
							if(alarmData != null){
								childCritical += alarmData.getCountCritical();
								childSerious += alarmData.getCountSerious();
								childWarn += alarmData.getCountWarn();
								childNotRecover += alarmData.getCountNotRecover();
								childRecover += alarmData.getCountRecover();
							}
						}
					}
				}
				...
				for(AlarmEvent alarmEvent:alarmEvents){
					...
					try {
						...
						if(resourceInstance != null && resourceInstance.getParentId() <= 0){
							...
							if(resourceInstance.getChildren() != null){
								for(ResourceInstance childResourceInstance : resourceInstance.getChildren()){
									if(childResourceInstance.getId()==Integer.parseInt(alarmEvent.getSourceID())){
										subList.add(alarmEvent);
									}
								}
							}
