...
/**
 * <li>文件名称: IDeviceTosApiImpl.java</li> <li>公　　司: 武汉美新翔盛科技有限公司</li> <li>版权所有:
 ...
 * @author lil
 */
public class IDeviceTosApiImpl implements IDeviceTosApi {

	private IDeviceTosDao deviceTosDao;

	/**
	 * @return the deviceTosDao
	 */
	public IDeviceTosDao getDeviceTosDao() {
		return deviceTosDao;
	}

	/**
	 * @param deviceTosDao
	 *            the deviceTosDao to set
	 */
	public void setDeviceTosDao(IDeviceTosDao deviceTosDao) {
		this.deviceTosDao = deviceTosDao;
	}

	@Override
	public NetflowPageBo deviceTosNetflowPageSelect(NetflowParamBo bo) {
		...
		// 获取某一设备的应用总流量
		Whole whole = deviceTosDao.getTotalTosNetflowsOfDevice(bo);
		...
		// 执行流量查询
		deviceTosDao.deviceTosNetflowPageSelect(page);
	...
	@Override
	public NetflowCharWrapper getDeviceTosChartData(NetflowParamBo bo) {
		...
		if (null != datas && !datas.isEmpty()) {
			for (NetflowBo e : datas) {
				...
				List<NetflowBo> bos = deviceTosDao.getDeviceTosChartData(bo);
