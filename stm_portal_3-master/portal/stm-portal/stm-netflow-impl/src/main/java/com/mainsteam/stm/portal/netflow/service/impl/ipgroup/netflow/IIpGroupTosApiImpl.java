...
/**
 * <li>文件名称: IIpGroupTosApiImpl.java</li> <li>公　　司: 武汉美新翔盛科技有限公司</li> <li>版权所有:
 ...
 * @author lil
 */
public class IIpGroupTosApiImpl implements IIpGroupTosApi {

	private IIpGroupTosDao ipGroupTosDao;

	/**
	 * @return the ipGroupTosDao
	 */
	public IIpGroupTosDao getIpGroupTosDao() {
		return ipGroupTosDao;
	}

	/**
	 * @param ipGroupTosDao
	 *            the ipGroupTosDao to set
	 */
	public void setIpGroupTosDao(IIpGroupTosDao ipGroupTosDao) {
		this.ipGroupTosDao = ipGroupTosDao;
	}

	@Override
	public NetflowPageBo ipGroupTosPageSelect(NetflowParamBo bo) {
		...
		// 获取某一设备的应用总流量
		Whole whole = ipGroupTosDao.getTotalIpGroupTosNetflows(bo);
		...
		// 执行流量查询
		ipGroupTosDao.ipGroupTosPageSelect(page);
	...
	@Override
	public NetflowCharWrapper getIpGroupAppChartData(NetflowParamBo bo) {
		...
		if (null != datas && !datas.isEmpty()) {
			for (NetflowBo e : datas) {
				...
				List<NetflowBo> bos = ipGroupTosDao.getIpGroupTosChartData(bo);
