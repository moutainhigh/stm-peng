...
/**
 * <li>文件名称: IIfIpgroupApiImpl.java</li>
 ...
 * @author   lil
 */
public class IIfIpgroupApiImpl implements IIfIpgroupApi {
	
	private IIfIpgroupDao ifIpgroupDao;
	...
	/**
	 * @return the ifIpgroupDao
	 */
	public IIfIpgroupDao getIfIpgroupDao() {
		return ifIpgroupDao;
	}

	/**
	 * @param ifIpgroupDao the ifIpgroupDao to set
	 */
	public void setIfIpgroupDao(IIfIpgroupDao ifIpgroupDao) {
		this.ifIpgroupDao = ifIpgroupDao;
	}

	@Override
	public NetflowPageBo ifIpgroupPageSelect(NetflowParamBo bo) {
...
//		}
		...
		//获取某一设备的应用总流量
//		long wholeFlows = ifIpgroupDao.getTotalIfIpgroupNetflows(bo);
		Whole w = ifIpgroupDao.getIfIpgroupTotals(bo);
		...
		//执行流量查询
		ifIpgroupDao.ifIpgroupPageSelect(page);
	...
	@Override
	public NetflowCharWrapper getIfIpgroupChartData(NetflowParamBo bo) {
		...
		if(null != datas && !datas.isEmpty()) {
			for(NetflowBo e : datas) {
				...
				List<NetflowBo> bos = ifIpgroupDao.getIfIpgroupChartData(bo);
