 ...
 * @author lil
 */
public class IDeviceAppApiImpl implements IDeviceAppApi {

	private IDeviceAppDao deviceAppDao;

	/**
	 * @return the deviceAppDao
	 */
	public IDeviceAppDao getDeviceAppDao() {
		return deviceAppDao;
	}

	/**
	 * @param deviceAppDao
	 *            the deviceAppDao to set
	 */
	public void setDeviceAppDao(IDeviceAppDao deviceAppDao) {
		this.deviceAppDao = deviceAppDao;
	}

	@Override
	public NetflowPageBo deviceAppNetflowPageSelect(NetflowParamBo bo) {
		...
		// 获取某一设备的应用总流量
		Whole whole = deviceAppDao.getTotalAppNetflowsOfDevice(bo);
		...
		// 执行流量查询
		deviceAppDao.deviceAppNetflowPageSelect(page);
		// 获取查询结果
	...
	@Override
	public NetflowCharWrapper findDeviceAppChartData(NetflowParamBo bo,
			List<NetflowBo> apps) {
		...
		if (null != apps && !apps.isEmpty()) {
			for (NetflowBo app : apps) {
				bo.setAppId(app.getAppId());
				List<NetflowBo> bos = deviceAppDao
						.queryDeviceAppNetflowOfTimepoint(bo);
				...
				list.add(new NetflowChartBo(app.getName(), data));
			}
		}
