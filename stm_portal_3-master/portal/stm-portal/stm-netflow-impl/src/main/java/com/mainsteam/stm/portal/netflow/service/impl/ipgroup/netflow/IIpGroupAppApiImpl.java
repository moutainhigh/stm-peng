...
/**
 * <li>文件名称: IIpGroupAppApiImpl.java</li> <li>公　　司: 武汉美新翔盛科技有限公司</li> <li>版权所有:
 ...
 * @author lil
 */
public class IIpGroupAppApiImpl implements IIpGroupAppApi {

	private IIpGroupAppDao ipGroupAppDao;

	/**
	 * @return the ipGroupAppDao
	 */
	public IIpGroupAppDao getIpGroupAppDao() {
		return ipGroupAppDao;
	}

	/**
	 * @param ipGroupAppDao
	 *            the ipGroupAppDao to set
	 */
	public void setIpGroupAppDao(IIpGroupAppDao ipGroupAppDao) {
		this.ipGroupAppDao = ipGroupAppDao;
	}

	@Override
	public NetflowPageBo ipGroupAppPageSelect(NetflowParamBo bo) {
		...
		// 获取某一设备的应用总流量
		Whole whole = ipGroupAppDao.getTotalIpGroupAppNetflows(bo);
		...
		// 执行流量查询
		ipGroupAppDao.ipGroupAppPageSelect(page);
	...
	public NetflowCharWrapper getIpGroupAppChartData(NetflowParamBo bo) {
		...
		if (null != datas && !datas.isEmpty()) {
			for (NetflowBo e : datas) {
				bo.setAppId(e.getAppId());
				List<NetflowBo> bos = ipGroupAppDao.getIpGroupAppChartData(bo);
