 ...
 * @author   lil
 */
public class IIfAppApiImpl implements IIfAppApi {
	
	private IIfAppDao ifAppDao;
	 ...
	 * @return the ifAppDao
	 */
	public IIfAppDao getIfAppDao() {
		return ifAppDao;
	}

	/**
	 * @param ifAppDao the ifAppDao to set
	 */
	public void setIfAppDao(IIfAppDao ifAppDao) {
		this.ifAppDao = ifAppDao;
	}

	@Override
	public NetflowPageBo ifAppPageSelect(NetflowParamBo bo) {
...
//		}
		...
		//获取某一设备的应用总流量
//		long wholeFlows = ifAppDao.getTotalIfAppNetflows(bo);
		Whole w = ifAppDao.getIfAppTotals(bo);
	...
	@Override
	public NetflowCharWrapper getIfAppChartData(NetflowParamBo bo) {
		...
		if(null != datas && !datas.isEmpty()) {
			for(NetflowBo e : datas) {
				...
				List<NetflowBo> bos = ifAppDao.getIfAppChartData(bo);
