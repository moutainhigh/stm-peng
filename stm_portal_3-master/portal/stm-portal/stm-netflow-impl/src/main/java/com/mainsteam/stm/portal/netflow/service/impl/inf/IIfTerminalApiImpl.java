...
/**
 * <li>文件名称: IIfTerminalApiImpl.java</li>
 ...
 * @author   lil
 */
public class IIfTerminalApiImpl implements IIfTerminalApi {
	
	private IIfTerminalDao ifTerminalDao;
	...
	/**
	 * @return the ifTerminalDao
	 */
	public IIfTerminalDao getIfTerminalDao() {
		return ifTerminalDao;
	}

	/**
	 * @param ifTerminalDao the ifTerminalDao to set
	 */
	public void setIfTerminalDao(IIfTerminalDao ifTerminalDao) {
		this.ifTerminalDao = ifTerminalDao;
	}

	@Override
	public NetflowPageBo ifTerminalPageSelect(NetflowParamBo bo) {
...
//		}
		...
		//获取某一设备的应用总流量
//		long wholeFlows = ifTerminalDao.getTotalIfTerminalNetflows(bo);
		Whole whole = ifTerminalDao.getIfTerminalTotals(bo);
		long wholeFlows = whole.getWholeFlows();
		long wholePackets = whole.getWholePackets();
		long wholeConnects = whole.getWholeConnects();
		...
		//执行流量查询
		ifTerminalDao.ifTerminalPageSelect(page);
	...
	@Override
	public NetflowCharWrapper getIfTerminalChartData(NetflowParamBo bo) {
		...
		if(null != datas && !datas.isEmpty()) {
			for(NetflowBo e : datas) {
				...
				List<NetflowBo> bos = ifTerminalDao.getIfTerminalChartData(bo);
